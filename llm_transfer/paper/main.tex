...
\textbf{[DRAFT PENDING — write after §4/§5 lock. Define \etaL{} = mean signed margin
$M(x)=f_y(x)-\max_{j\neq y}f_j(x)$ over correctly-classified clean inputs, divided by the mean
dual-norm input gradient $\lVert\nabla_x M\rVert_q$ ($q{=}1$ for $\ell_\infty$, $q{=}2$ for $\ell_2$).
State gauge-freeness (invariant under $f\to cf$) and the certificate reading (robust radius
$\geq \etaL$). List the four contributions above.]}
...
\textbf{[DRAFT PENDING — brief recap of the prior CIFAR result, cited as the origin.]}
...
\section{Weak-attack robustness is an artifact}
\label{sec:weakattack}
% PROMOTE from bonus to full section, pending the novelty verification (agent ac324...) + added controls.
% Verified core: every non-AT CLIP tower has FGSM robust acc 0.16-0.46 at eps 2/255-4/255 but APGD=PGD40=0.0.
% Planned additions (if novel): FGSM vs PGD-k monotonicity k=1,2,5,10,20,40; Square black-box; per-tower gradient-norm/loss-landscape diagnostic.
\textbf{[DRAFT PENDING novelty verification. Core result: on every non-robust tower, single-step FGSM
reports 16--46\% robust accuracy that APGD and PGD-40 drive to zero, so any ``more invariant is more
robust'' reading measured with a weak attack does not survive AutoAttack.]}
