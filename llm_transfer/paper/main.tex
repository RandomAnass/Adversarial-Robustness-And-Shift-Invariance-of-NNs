...
certified radius more than twice as strongly as shift-consistency does. The ranking survives removing
the effect of adversarial training as a covariate, whereas a cosine-similarity form of consistency
does not: it only appeared to work because it detects adversarial training. In instruction-tuned language models the two
...
more than twice as strongly as shift-consistency; the tower ranking survives removing adversarial
training as a covariate, while a cosine form of consistency does not. Second, in instruction-tuned language models the
...
\caption{Tower-level dissociation across the eleven-tower panel. \textbf{Left:} the attack-free ratio
\etaL{}, computed on clean images, ranks the towers by AutoAttack robust accuracy (Pearson $+0.95$);
adversarially trained towers in orange, ordinary towers in blue. \textbf{Right:} after removing an
adversarial-training indicator as a covariate, \etaL{} still orders robustness ($+0.51$), while
cosine-consistency collapses ($+0.15$) because its apparent signal was detection of adversarial
training, and prediction-agreement consistency is not significant.}
\label{fig:tower}
\end{figure}
...
meaning-changing edits are, in these models, orthogonal axes (Figure~\ref{fig:ortho}).

\begin{figure}[t]
\centering
\includegraphics[width=0.5\linewidth]{figures/fig5_orthogonality.pdf}
\caption{In a language model the sensitivity ratio \etaL{} and the excessive-invariance radius \rhoG{}
are uncorrelated across prompts (Spearman $-0.03$), so they are independent robustness axes rather than
two views of one quantity.}
\label{fig:ortho}
\end{figure}
