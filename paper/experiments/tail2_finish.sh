#!/usr/bin/env bash
# Replacement tail (detached). The previous tail's Phase C ran at K=3 and thrashed CIFAR memory
# (both 48 GiB cards maxed, ~2/24 in 7h). cifar_at_parallel.py's cap now forces CIFAR -> K=2, so this
# re-launches Phase C (resumes from the saved partials) -> Experiment #3 -> all_gpu_done.marker.
set -u
...
RES=paper/results
LOG=$RES/tail2_finish.log
say(){ echo "[tail2] $(date -u +%FT%TZ) $*" | tee -a "$LOG"; }
halt(){ say "HALT: $*"; echo "$*" > $RES/CHAIN_HALT.marker; exit 1; }
have(){ n=$(ls "$1" 2>/dev/null | grep -c '\.json$'); say "$3: $n/$2"; [ "$n" -ge "$2" ]; }

say "START (pid $$); Phase C at corrected K (CIFAR auto-capped to 2), resumes from saved partials"
...
say "ALL_GPU_DONE (Phase C 24/48 + Exp3). Analysis/integration -> agent."
