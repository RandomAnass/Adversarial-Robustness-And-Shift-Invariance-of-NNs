...
\paragraph{A held-out operator from the contrary literature: TIPS.} The closest work claiming the opposite correlation, that better shift-invariance goes with better adversarial robustness, is the learnable translation-invariant polyphase operator TIPS \citep{saha2024improving}. We port TIPS as a further arm of the ResNet dissection, faithfully as a downsampling operator at the same three sites as the other arms (its auxiliary shift-consistency losses are omitted so that the shared PGD adversarial-training recipe isolates the operator; TIPS adds two depthwise convolutions, so this arm is near- rather than exactly capacity-matched, $+0.16\%$ parameters). The ported operator accordingly inherits neither the full method's consistency gain nor its weak-attack advantage under standard training (Table~\ref{tab:weakattack}), consistent with the auxiliary losses rather than the operator driving those reported gains; our claims concern the operator. Two results. First, under adversarial training the TIPS operator is a \emph{good} operator: robust accuracy $0.420$ at width $1.0$ ($0.389$ at width $0.5$), comparable to anti-aliased blur-pooling, with high consistency ($0.95$) and no gradient masking. Its robustness is exactly what the ratio predicts: the $\eta/L$ law fitted on the twelve cells \emph{without} TIPS predicts its AutoAttack accuracy to $+0.028$ and $-0.014$ at the two widths, an out-of-sample test on an operator we did not design. With TIPS included (fourteen cells, seven operators) the threat-matched ratio predicts robust accuracy at Pearson $+0.88$ (partial correlation $+0.88$ controlling for clean accuracy) while consistency remains anti-correlated ($-0.79$). Second, TIPS sharpens the consistency claim: consistency is uninformative in \emph{both} directions. The exactly invariant adaptive-polyphase arm has consistency $1.0$ and the lowest robust accuracy; TIPS has consistency $0.95$ and among the highest. Knowing a model's shift-consistency tells one nearly nothing about its robustness, while its threat-matched $\eta/L$ places both operators on the same line.
