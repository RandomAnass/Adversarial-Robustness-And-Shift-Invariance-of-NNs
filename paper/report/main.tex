...
\]
The radial term is Euler's identity, $\langle\nabla M(x),x\rangle=M(x)$, which gives the floor $\|\nabla M(x)\|_2\ge M(x)/\|x\|_2$ and the cap $\eta/L\le\sup_i\|x_i\|_2$. The upper envelope $\|\nabla M(x_i)\|_2\le\kappa\,M(x_i)/\|x_i\|_2$ holds exactly when the tangential factor is bounded, $\|\nabla_{\mathbb S}\log m(u_i)\|_2\le\sqrt{\kappa^2-1}$; equivalently $\kappa_i=\sec\angle(\nabla M(x_i),x_i)$.
\end{proposition}
\begin{proof}
...
This backs the data-condition-number reading of Remark~\ref{rem:condnum}: the tangential factor of Proposition~\ref{prop:split} is bounded by the normalized margin the gradient-descent solution actually achieves, with no appeal to global optimality.
