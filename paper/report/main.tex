...
\paragraph{The contrary correlation lives only under weak attacks without adversarial training.} The invariance-helps-robustness reports \citep{saha2024improving,wang2025bridging} measure standard-trained models under gradient-based attacks, FGSM and PGD (rotation- and scale-equivariant operators rather than pooling operators in the latter case), without AutoAttack and without adversarial training. We reproduce that evaluation regime on the shift axis, on our capacity-matched arms plus TIPS (standard training, FGSM/PGD/AutoAttack at $\varepsilon\in\{1,2,4,8\}/255$; Table~\ref{tab:weakattack}). Under FGSM at $1$--$2/255$ the invariance-helps ordering \emph{does} appear: the exactly invariant and shift-augmented arms sit above the standard baseline. Under AutoAttack the same models are at most $0.006$ robust accuracy at $\varepsilon=2/255$, for every arm including TIPS; \citet{saha2024improving} evaluate at budgets up to $192/255$, where AutoAttack robust accuracy of any standard-trained arm is a fortiori zero, so our grid bounds theirs from below. The ordering those works report does appear, but it is a property of weak attacks on undefended models, a regime with no adversarial robustness for any operator; under adversarial training, where robustness is nonzero, the ordering is governed by the threat-matched $\eta/L$ and not by invariance.
...
\begin{table}[h]\centering
\caption{Standard-trained arms under weak and strong attacks (width $1.0$; single seed; $\ell_\infty$ budgets in $/255$; AutoAttack on $512$ points, binomial $95\%$ half-width $\approx0.007$ near zero, so the collapse conclusion is insensitive to the sample). The FGSM ordering at $\varepsilon{=}1$ reproduces the invariance-helps reading for the built-in-invariance arms (exact-APS and shift-augmented above standard); the AutoAttack columns show every arm is non-robust in this regime.}
...
TIPS          & 0.929 & 0.974 & 0.459 & 0.229 & 0.261 & 0.000 & 0.000 \\
\bottomrule
\end{tabular}
\end{table}
