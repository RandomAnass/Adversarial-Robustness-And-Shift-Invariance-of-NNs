...
\begin{abstract}
Making a convolutional network more shift-invariant has been reported both to reduce its adversarial robustness \citep{ge2021shift} and to improve it \citep{grabinski2022aliasing,saha2024improving}. We show that these are two regimes of one quantity: the margin of the discriminative signal that is preserved when the data is projected onto the invariant feature space, measured against that feature's sensitivity, written as a margin-to-Lipschitz ratio $\eta/L$. When the projection keeps a large ratio, invariance and robustness coexist; when it leaves only a small margin, they trade off. For a shift-invariant linear classifier the ratio is exact and recovers the $1/\sqrt{\dd}$ robustness collapse of \citet{ge2021shift} for a localized signal, while for frequency-coded classes a convolution-plus-pooling layer with a nonlinearity computes power-spectrum features that separate them when no linear classifier can. The ratio is also a robustness certificate, $r_2\ge\eta/L$, which we complete to a two-sided bracket under an explicit reachability condition, and it places the apparent counterexample of \citet{kamath2021canwehaveitall} as a small-margin corner rather than a contradiction. On the optimization side, shift-invariance cannot raise the margin on orbit-closed data, so any robustness gain there comes through the Lipschitz term, which we prove in the lazy regime. Empirically, across standard and adversarial training, three datasets, two threat models, and a PreActResNet-18 backbone, the threat-matched ratio predicts the adversarial robust radius and AutoAttack robust accuracy, whereas shift-consistency does not by itself, and under adversarial training it is negatively related to robustness; used as an attack-free rule for selecting an invariance operator, $\eta/L$ picks the most robust one in every condition while shift-consistency picks the least. The mechanism is a decomposition of each operator's effect into a margin term and a Lipschitz term: a graded anti-aliasing sweep improves robustness at matched clean accuracy, while exact invariance hurts by collapsing the margin, a single change that lowers clean accuracy and robustness together. A homogeneity identity gives a structural lower envelope $\|\nabla M(x)\|\ge M(x)/\|x\|$ for bias-free homogeneous networks, so the coupling is not purely an optimization artifact, though this envelope is loose at the operating point. The obstruction to training $\eta/L$ is instead empirical and sharp: four controlled interventions that penalize or directly maximize $\eta/L$ all collapse the margin and leave robustness flat or worse, and because the ratio is scale-invariant its maximizer is the constant classifier. So $\eta/L$ diagnoses robustness rather than serving as a training target.
\end{abstract}
...
The certificate $r_2\ge\eta/L$ makes $\eta/L$ a target one might hope to train. We argue it is a diagnostic rather than a trainable objective, from three angles: a homogeneity identity puts a structural floor under the input-gradient norm (so the coupling is not purely an optimizer artifact), the ratio is scale-invariant (so its direct maximizer is degenerate), and four controlled interventions (\S\ref{sec:exp}) confirm that every attempt to lower or maximize it collapses the margin. For a $k$-class score $f$ with true label $y$, write $M(x)=f_y(x)-\max_{j\neq y}f_j(x)$ for the signed logit margin and $\eta=\mathbb E[M]$, $L=\mathbb E\|\nabla M\|$ for its dataset means, the same $(\eta,L)$ the dissection of \S\ref{sec:exp} measures.

\begin{theorem}[Homogeneous self-limiting bound]\label{thm:coupling}
Let $f:\R^\dd\to\R^k$ be a bias-free network whose activations are all positively homogeneous of degree one ($\sigma(ct)=c\,\sigma(t)$ for $c>0$; ReLU, leaky-ReLU, absolute value, maxout). Then $M=f_y-\max_{j\neq y}f_j$ is positively homogeneous of degree one, and at every differentiability point $x$,
\[
\langle\nabla M(x),x\rangle=M(x)\ \text{(Euler)},\qquad\text{hence}\qquad
\|\nabla M(x)\|_2\ \ge\ \frac{M(x)}{\|x\|_2}\quad\text{and}\quad\frac{M(x)}{\|\nabla M(x)\|_2}\le\|x\|_2,
\]
with the threat-matched form $\|\nabla M(x)\|_1\ge M(x)/\|x\|_\infty$. Consequently the first-order ratio is capped by the input norm, $\eta/L\le\sup_i\|x_i\|_2$, independently of training; on data of comparable norm the scatter $(M(x_i),\|\nabla M(x_i)\|_2)$ lies on or above the line through the origin of slope $1/\max_i\|x_i\|_2$, a deterministic positive lower envelope; and $\|\nabla M\|$ cannot be driven below $M/\|x\|$ without lowering $M$.
\end{theorem}
\begin{proof}
Each logit is built from bias-free affine maps and degree-1 positively homogeneous activations, so $f_j(cx)=cf_j(x)$ for $c>0$ by induction over layers; a maximum of degree-1 positively homogeneous functions is again one, so $M(cx)=cM(x)$. Differentiating in $c$ at $c=1$ gives Euler's identity $\langle\nabla M(x),x\rangle=M(x)$ at each differentiability point (a.e.\ by Rademacher; on the measure-zero nonsmooth set of a ReLU network a Clarke subgradient $v$ with $\langle v,x\rangle=M(x)$ serves). Cauchy--Schwarz gives $M(x)=\langle\nabla M(x),x\rangle\le\|\nabla M(x)\|_2\,\|x\|_2$, and Hölder the $\ell_1/\ell_\infty$ form; rearranging proves the claims. \end{proof}

The envelope shows the margin--sensitivity coupling is not purely an optimizer artifact: there is an architecture-imposed floor below which the gradient cannot fall without the margin falling. Two honest limits bound how much this explains. First, the envelope is \emph{loose at the operating point}: for our trained models $\|\nabla M\|_2$ sits well above $M/\|x\|_2$ (the floor is a fraction of the measured value), so the bound permits large reductions in the gradient before it binds and does not by itself forbid raising $\eta/L$; the binding obstruction to training $\eta/L$ is the scale-invariance of the ratio (Lemma~\ref{lem:ratiodegen}) together with the four interventions of \S\ref{sec:exp}, not this envelope. Second, the identity is exact only for bias-free homogeneous networks; additive biases, batch normalization, and smooth activations break it, so on the PreActResNet-18 models it is an idealized mechanism, not an exact law. The coupling nonetheless persists across activations empirically, which points to the architecture-independent capacity ceiling below (Remark~\ref{rem:capacity}).

\begin{remark}[Capacity ceiling, after \citealp{bubeck2021law}]\label{rem:capacity}
The same ceiling has an architecture-independent capacity reading. On isoperimetric data with label noise, the universal law of robustness \citep{bubeck2021law} forces any $p$-parameter model that interpolates $n$ points in $\R^\dd$ to have global Lipschitz constant $\tilde\Omega(\sqrt{n\dd/p})$. Applying it to the clipped margin $h=\mathrm{clip}(M/\eta,-1,1)$, which fits the labels and has $\mathrm{Lip}(h)\le L/\eta$, gives $\eta/L\le\tilde O(\sqrt{p/(n\dd)})$, so a certified ratio of order one needs $p\gtrsim n\dd$ parameters. This depends only on $(p,n,\dd)$ and not on the activation, which is why the coupling held for ReLU, SiLU, and GELU alike. Two caveats keep it honest: the law bounds the \emph{global} Lipschitz constant whereas our $L$ is the \emph{local} gradient norm at the data, and it assumes label noise (a memorization regime), so we read it as the capacity context behind the coupling, not as a tight account of the measured local ratio. The order of magnitude is plausible, $\sqrt{p/(n\dd)}\approx0.27$ for a PreActResNet-18 on CIFAR-10.
\end{remark}

A second degeneracy rules out the obvious workaround of maximizing the ratio directly.
\begin{lemma}[Scale-invariance of the ratio]\label{lem:ratiodegen}
The per-sample ratio $M(x)/\|\nabla M(x)\|$ is invariant to positive rescaling of the score, $f\mapsto cf$ for $c>0$, since numerator and denominator scale together. Hence an objective that maximizes $\eta/L$ (equivalently, penalizes $\|\nabla M\|/M$ over correctly classified points) is blind to the absolute margin and admits the constant classifier $\nabla M\equiv0$ as a global optimizer, so maximizing $\eta/L$ directly is ill-posed as a training objective.
\end{lemma}
\begin{proof}
Under $f\mapsto cf$ both $M$ and $\nabla M$ scale by $c$, leaving the ratio unchanged; the penalty $\mathbb E[\|\nabla M\|/M]\ge0$ attains its infimum $0$ only as $\nabla M\to0$, i.e.\ at a constant score, which classifies at chance. This is the constant-function degeneracy flagged by \citet{tsuzuku2018lipschitz}. \end{proof}

Together, Theorem~\ref{thm:coupling} and Lemma~\ref{lem:ratiodegen} predict a no-go for first-order training of $\eta/L$: penalizing $\|\nabla M\|$ must shrink the margin, and maximizing the ratio collapses to the constant classifier. Section~\ref{sec:exp} tests this prediction with four controlled interventions, all of which collapse the margin exactly as predicted.
...
A mechanism for why shift-consistency is the unreliable signal is visible in the consistency axis. A ResNet backbone is already strongly shift-invariant, so over the four capacity-matched arms alone shift-consistency is squeezed into $[0.93,1.00]$ and its anti-prediction washes out (Pearson $-0.43$ over eight cells, not significant); only when the low-invariance arms widen the axis to $[0.86,1.00]$ does the anti-prediction become significant ($-0.81$). The margin-to-Lipschitz ratio does not saturate this way and separates the arms throughout: on a strong backbone shift-consistency is nearly constant while $\eta/L$ still tracks robustness.
...
$r_2=\eta/L$ exactly when $\kappa=1$.
...
\begin{theorem}[Lazy-regime cluster advantage: invariance lowers the certified Lipschitz constant]\label{thm:lazy-cluster}
...
\item therefore $L_{\mathrm{inv}}\le L_{\mathrm{unc}}$, and at the common unit margin $\eta$
  ...
  $\eta/L_{\mathrm{inv}}\ge\eta/L_{\mathrm{unc}}$. The gap is the discarded non-invariant
  energy $\|(I-\Pi_G)f_{\mathrm{unc}}\|_K^2$ (the orbit-variance of the unconstrained
  solution), and is strict unless $f_{\mathrm{unc}}$ is already shift-invariant.
\end{enumerate}
\end{theorem}
...
norm gap exactly $\|(I-\Pi_G)f_{\mathrm{unc}}\|_K^2$, which vanishes iff
$f_{\mathrm{unc}}=\Pi_G f_{\mathrm{unc}}$ is already invariant.
\end{proof}
...
result thus proves the cluster-geometry advantage in the lazy, smooth-activation regime,
exactly where the synthetic stress test shows the largest tied-versus-unconstrained
$\eta/L$ gap, while the rich (small-init) and bare-ReLU trajectories, where the fixed-kernel
picture breaks down \citep{chizat2019lazy}, stay open.
\end{remark}
