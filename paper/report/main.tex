...
\subsection{The margin--sensitivity coupling is structural: no first-order regularizer raises \texorpdfstring{$\eta/L$}{eta/L}}\label{sec:coupling}
The certificate $r_2\ge\eta/L$ makes $\eta/L$ a target one might hope to train. We show it cannot be trained by any first-order penalty: the margin and its input-gradient norm are coupled by an identity, so lowering the gradient lowers the margin in lockstep. For a $k$-class score $f$ with true label $y$, write $M(x)=f_y(x)-\max_{j\neq y}f_j(x)$ for the signed logit margin and $\eta=\mathbb E[M]$, $L=\mathbb E\|\nabla M\|$ for its dataset means, the same $(\eta,L)$ the dissection of \S\ref{sec:exp} measures.
...
\]
with the threat-matched form $\|\nabla M(x)\|_1\ge M(x)/\|x\|_\infty$. Consequently the first-order ratio is capped by the input norm, $\eta/L\le\sup_i\|x_i\|_2$, independently of training; on data of comparable norm the scatter $(M(x_i),\|\nabla M(x_i)\|_2)$ lies on or above the line through the origin of slope $1/\max_i\|x_i\|_2$, a deterministic positive lower envelope; and any penalty that drives $\|\nabla M\|$ below $M/\|x\|$ is infeasible, so it can lower $\|\nabla M\|$ only by lowering $M$ in lockstep.
\end{theorem}
\begin{proof}
...
This is the structural reason the margin--sensitivity coupling is positive (\S\ref{sec:exp}): the envelope forces larger margin to come with larger gradient, so the coupling is a property of the architecture, not an artifact of the optimizer. The bound is exact for bias-free homogeneous networks; additive biases, batch normalization, and smooth activations (SiLU, GELU) break Euler's identity, so for those it is the clean idealized mechanism rather than an exact identity. The coupling nonetheless persists across activations empirically, which points to the architecture-independent capacity cause.
