...
Table~\ref{tab:weakattack} gives the numbers behind the weak-versus-strong contrast of \S\ref{sec:exp}: the capacity-matched ResNet arms plus TIPS, \emph{standard}-trained (no adversarial training), attacked with FGSM, PGD-10, and AutoAttack at small $\ell_\infty$ budgets, the protocol of the invariance-helps reports \citep{saha2024improving,wang2025bridging}. Under FGSM at $1/255$ the more-invariant arms sit above the standard baseline, reproducing that literature's ordering; under AutoAttack every arm is at or near zero robust accuracy by $\varepsilon=2/255$, so the ordering ranks models that are all non-robust. FGSM overestimates robust accuracy here by up to a factor of $40$ relative to PGD-10 at the same budget.

\begin{table}[h]\centering
\caption{Standard-trained arms under weak and strong attacks (width $1.0$; seed $0$; $\ell_\infty$ budgets in $/255$; AutoAttack on $512$ points). The FGSM column ordering at $\varepsilon{=}1$ reproduces the invariance-helps reading (exact-APS and shift-augmented above standard); the AutoAttack columns show every arm is non-robust in this regime.}
