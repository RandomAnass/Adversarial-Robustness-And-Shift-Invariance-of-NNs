...
\paragraph{Under adversarial training.} To test whether the picture survives a strong standardized attack, we PGD-adversarially train \citep{madry2018towards} the same four arms ($\ell_\infty$, $\epsilon=8/255$, $7$ steps; two widths, two seeds) and evaluate with AutoAttack at the RobustBench thresholds (Table~\ref{tab:cifar-at}). Adversarial training makes AutoAttack informative: robust accuracy is now $26$--$42\%$ at $\ell_\infty=8/255$, with no gradient masking (AutoAttack stays below matched PGD for every arm). Three things hold. (i) Shift-consistency remains strongly \emph{anti}-correlated with robustness (Pearson $-0.88$ against AutoAttack); the exactly invariant model has consistency $1.0$ and the lowest robust accuracy of the four. (ii) The $\eta/L$ criterion still predicts robustness once its norm is matched to the threat: the $\ell_\infty$-matched ratio $\eta/\|\nabla M\|_1$, using the dual norm of the $\ell_\infty$ attack, tracks AutoAttack robust accuracy with Pearson $0.88$ (Figure~\ref{fig:cifar-at}), whereas the $\ell_2$ ratio ($0.55$) and the $\ell_2$ robust radius (near zero) do not, because adversarial training robustifies in $\ell_\infty$, not $\ell_2$. (iii) The robustness ordering is anti-aliased $\ge$ standard $\approx$ shift-augmented $>$ exact cyclic, the same direction as standard training. One confound: the exactly invariant arm reaches lower clean accuracy under adversarial training ($0.57$--$0.63$ vs.\ $0.69$--$0.78$), so part of its robustness deficit reflects reduced fitting rather than margin geometry alone; the consistency anti-prediction and the threat-matched $\eta/L$ law are not explained by clean accuracy.
...
\caption{Per-sample structure on the PreActResNet-18 cells. Left: within one adversarially trained model, the first-order ratio $M/\|\nabla M\|_2$ predicts each point's own $\ell_2$ robust radius (Spearman $+0.91$), and the within-cell rank correlation is positive in every cell across standard training, adversarial training, and CIFAR-100. Right: the within-cell coupling between margin $M$ and input-gradient norm $\|\nabla M\|_2$ is negative under standard training and positive after adversarial training, in every cell.}
\label{fig:persample}
\end{figure}
