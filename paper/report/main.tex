...
A Threat-Matched Margin-to-Lipschitz Diagnostic}
...
\begin{abstract}
Making a convolutional network more shift-invariant has been reported both to reduce its adversarial robustness \citep{ge2021shift} and to improve it \citep{grabinski2022aliasing,saha2024improving}. We show these are two regimes of one quantity, a margin-to-Lipschitz ratio $\eta/L$: the margin of the discriminative signal that survives projection onto the invariant feature space, measured against that feature's sensitivity. A large ratio lets invariance and robustness coexist; a small surviving margin forces a trade-off. The ratio is gauge-free: rescaling the score leaves it unchanged while moving the margin and the Lipschitz constant separately, so robustness attaches to $\eta/L$, not to either term alone.

Structurally, for a shift-invariant linear classifier the ratio is exact and recovers the $1/\sqrt{\dd}$ robustness collapse of \citet{ge2021shift} for a localized signal, while for frequency-coded classes a convolution-plus-pooling layer with a nonlinearity computes power-spectrum features that separate classes no linear classifier can. The ratio is also a robustness certificate, $r_2\ge\eta/L$, which we complete to a two-sided bracket under an explicit reachability condition; it places the apparent counterexample of \citet{kamath2021canwehaveitall} as a small-margin corner rather than a contradiction. On the optimization side, shift-invariance cannot raise the margin on orbit-closed data, and in the lazy regime it cannot lower the minimum-norm certificate either (the invariant and unconstrained minimum-norm interpolants coincide), so any robustness gain there is a property of the realized finite-width solution, read through its $\eta/L$, not of the minimum-norm certificate.

Empirically, across standard and adversarial training, three datasets, two threat models, and a PreActResNet-18 backbone (with a CIFAR-100 check, and an ImageNet-100 check where clean accuracy is matched across arms so the dissociation is free of the fitting confound), the threat-matched ratio predicts the adversarial robust radius and AutoAttack robust accuracy; fitted without the learnable TIPS operator from the contrary literature, it predicts that operator's robustness out of sample. Shift-consistency is uninformative in both directions once the invariance axis is sampled widely enough: the exactly invariant operator has perfect consistency yet the lowest robustness, the high-consistency TIPS operator is middling rather than least robust, and the most robust operators are the least consistent, so under adversarial training consistency is negatively related to robustness, whereas the threat-matched ratio predicts robust accuracy even after controlling for clean accuracy (partial correlation $+0.89$). The invariance-helps ordering reported in prior work reproduces under FGSM on standard-trained models and collapses to zero under AutoAttack already at budgets below those works use. The mechanism is a decomposition of each operator's effect into a margin term and a Lipschitz term: a graded anti-aliasing sweep improves robustness at matched clean accuracy, while exact invariance hurts by collapsing the margin, one change that lowers clean accuracy and robustness together. Used as an attack-free rule for choosing an invariance operator, $\eta/L$, on par with clean accuracy, picks the most or near-most robust one, while shift-consistency picks the least.

Yet $\eta/L$ is a diagnostic, not a trainable target. An exact polar identity splits the margin gradient of a bias-free homogeneous network into a radial floor $M(x)/\|x\|$ and a tangential factor, exactly characterizing the coupling; that factor is the data condition number (exact for linear max-margin) and is large at the operating point. The obstruction to training $\eta/L$ is therefore empirical and sharp: four controlled interventions that penalize or directly maximize $\eta/L$ all collapse the margin and leave robustness flat or worse, and because the ratio is scale-invariant its maximizer is the constant classifier. Finally, the diagnostic extends to a second axis, the training \emph{data} rather than the architecture: adversarial training augmented with diffusion-generated images yields models whose threat-matched $\eta/L$ tracks AutoAttack robust accuracy (Pearson $+0.85$ over twelve models spanning four data doses and three seeds) while the mismatched ratio anti-predicts it, and this axis cleanly exposes that the apparent margin-versus-Lipschitz attribution is confounded by gauge choice and by robust overfitting, which read through the ratio is itself an $\eta/L$ collapse that the added data prevents. Swept across adversarial-training budgets, all sixteen models collapse onto one curve in the budget-normalized ratio $\eta/(L\varepsilon)$ (Pearson $+0.94$), and the per-sample certified fraction predicts AutoAttack robust accuracy at $+0.998$, absorbing the data intervention's remaining offset. So $\eta/L$ diagnoses robustness rather than serving as a trainable target.
\end{abstract}

\section{Introduction}\label{sec:intro}
Let an image classifier be \emph{shift-invariant} if its prediction is unchanged by circular translations of the input. \citet{ge2021shift} showed, for a binary problem in which each class is a single image and all its shifts, that a fully shift-invariant convolutional network needs an $\ell_2$ perturbation of order $1/\sqrt{\dd}$ to be fooled while a fully connected network needs order $1$, and that this gap appears on real datasets. A separate line of architectural work points the other way. Anti-aliased pooling improves shift-consistency (how often the prediction is left unchanged by a shift) together with clean accuracy and corruption robustness \citep{zhang2019making}, and reducing aliasing has since been reported to improve \emph{adversarial} robustness as well \citep{grabinski2022aliasing,rodriguezmunoz2022aliasing,saha2024improving}: operators that make a network more shift-invariant also make it more robust. A parallel line proves a trade-off between \emph{spatial} (geometric) and adversarial robustness \citep{kamath2021canwehaveitall,tramer2019multiple}.

These statements are not contradictory; they are different regimes of one quantity, a margin-to-Lipschitz ratio $\eta/L$: how much discriminative margin $\eta$ survives the invariance, divided by how sensitive $L$ that surviving feature is to a change in the input. When the ratio is large, invariance and robustness coexist; when the surviving margin is small, they trade off. Because an attack in one norm is resisted by sensitivity measured in the dual norm, we always match the Lipschitz term to the attack, and call the result the \emph{threat-matched} $\eta/L$. Our contributions:
...
\item \textbf{Optimization: a margin-free result and the open selection problem (\S\ref{sec:opt}).} The structural theory establishes that robust shift-invariant classifiers exist; whether gradient descent reaches them is separate. We give the exact rank of a shift orbit (its number of nonzero Fourier coefficients), and prove that on orbit-closed data (a class set closed under all shifts) shift-invariance is \emph{margin-free}: the invariant and unconstrained two-layer-ReLU max-margin values coincide (Corollary~\ref{cor:no-gap-var}), so weight sharing cannot raise the margin. A direct synthetic test confirms this on generic orbit data, where gradient descent self-symmetrizes (the trained network becomes shift-invariant on its own) and the invariant and unconstrained networks reach the same robustness, while on near-orthogonal cluster-geometry data the invariant architecture reaches a larger \emph{realized} $\eta/L$. In the lazy (neural-tangent-kernel, NTK) regime orbit-averaging is a non-expansive projection in the tangent reproducing-kernel Hilbert space (RKHS) and the invariant and unconstrained minimum-norm certificates \emph{coincide} on orbit-closed data, so this advantage is a realized, finite-width effect rather than a minimum-norm gap, leaving the rich-regime trajectory open.
\item \textbf{Experiments (\S\ref{sec:exp}).} A training-free test confirms the structural predictions; a controlled, capacity-matched architectural dissection on MNIST, Fashion-MNIST, and CIFAR-10 confirms that $\eta/L$, not shift-consistency, predicts the robust radius, and decomposes each invariance operator's effect into a margin term and a Lipschitz term. Under adversarial training with projected gradient descent (PGD) the same two laws hold across three datasets and two threat models, scale to a PreActResNet-18 backbone on CIFAR-10 and CIFAR-100, where they also hold per sample as a pointwise certificate, and hold at ImageNet-100 resolution with clean accuracy matched across arms; the law fitted without the TIPS operator of \citet{saha2024improving} predicts that operator's robustness out of sample, and the contrary invariance-helps ordering reproduces only under weak attacks on undefended models. As an attack-free selection rule, $\eta/L$ (like clean accuracy) picks the most or near-most robust operator while shift-consistency picks the least. Four controlled interventions then show $\eta/L$ cannot be trained: every first-order attempt to raise it collapses the margin, leaving robustness flat or worse. Finally, changing the \emph{data} rather than the architecture, through diffusion-augmented adversarial training, reproduces the threat-matched law on a new axis and exposes two ways the margin-versus-Lipschitz split misleads: gauge choice and robust overfitting. Swept across attack budgets, the models collapse onto one budget-normalized $\eta/(L\varepsilon)$ curve and a per-sample certified fraction predicts robust accuracy, absorbing the cross-axis offset; and the two operators from the contrary literature (the learnable polyphase operator of \citet{saha2024improving} and, retrained with its own code, the rotation-equivariant architecture of \citet{wang2025bridging}) turn out robust only under weak attacks, their advantage vanishing under AutoAttack.
\end{itemize}
\paragraph{One organizing principle.} The ratio $\eta/L$ is \emph{gauge-free}: rescaling the score $f\mapsto cf$ multiplies both the margin $\eta$ and the local Lipschitz constant $L$ by $c$, leaving the ratio (and the input-space radius it lower-bounds) unchanged (Lemma~\ref{lem:ratiodegen}). The margin and the Lipschitz constant are individually \emph{gauge-dependent}, so attributing a robustness change to ``larger margin'' or ``smaller Lipschitz'' separately is well-posed only once a gauge is fixed. We therefore anchor the robustness attribution on the gauge-invariant quantities, $\eta/L$ and the certified radius, and read the empirical margin/Lipschitz split (\S\ref{sec:exp}) at the training-fixed logit scale, where the per-term decomposition is well-defined. This single principle organizes the paper: invariance changes robustness only through $\eta/L$, and in the orbit-closed regime where it cannot move the minimum-norm $\eta/L$ at all, any empirical advantage is a property of the \emph{realized} finite-width solution rather than of the norm certificate. It also reaches past architecture: when we intervene on the training \emph{data} instead (\S\ref{sec:exp}), diffusion-augmented adversarial training obeys the same threat-matched $\eta/L$ law within that axis, a second and independent test of the diagnostic (though the two axes do not share one regression line; \S\ref{sec:exp}). And it is why $\eta/L$ is a diagnostic rather than a trainable target (\S\ref{sec:coupling}).

Full proofs are in Appendix~\ref{app:proofs}.

\section{Preliminaries}\label{sec:prelim}
We work with one-dimensional signals $x\in\R^{\dd}$; the two-dimensional case replaces $\Z_{\dd}$ by $\Z_{d_1}\times\Z_{d_2}$ and the cyclic discrete Fourier transform (DFT) by its two-dimensional analogue, without change to the statements. The cyclic group $G=\Z_{\dd}$ acts by the circular shift $(S_s x)_i = x_{(i-s)\bmod \dd}$; each $S_s$ is an orthogonal permutation. The \emph{orbit} of $x$ is $\mathcal{O}(x)=\{S_s x\}_{s\in G}$, the set of all its shifts, and a class set is \emph{orbit-closed} if it is closed under all shifts.

\paragraph{Invariant and AC subspaces.} Let $\Vinv=\{v: S_s v=v\ \forall s\}$ and $\Vac=\Vinv^{\perp}$. The orthogonal projector onto $\Vinv$ is $\Pi_{\mathrm{inv}}=\tfrac1\dd \mathbf 1\mathbf 1^\top$, so $\Vinv=\mathrm{span}(\mathbf 1)$ is the constant (DC, or zero-frequency) line and $\Vac=\{v:\sum_i v_i=0\}$ is its zero-mean (AC) complement. Write $\fdc(x)=\tfrac{1}{\sqrt\dd}\sum_i x_i$ for the unit DC functional. All $S_s$ are simultaneously diagonalized by the DFT; a shift multiplies Fourier coefficient $k$ by a unit-modulus phase, so $|\hat x_k|^2$ is shift-invariant.
...
If the score is $L$-Lipschitz toward the boundary and admits a co-Lipschitz (lower-Lipschitz) constant $\alpha$ along a path reaching it, then $\eta/L\le r_2\le\eta/\alpha$ with condition number $\kappa:=L/\alpha\ge1$, so $\eta/L$ determines the robust radius up to $\kappa$. The bracket is exact ($r_2=\eta/\|w\|_2$, $\kappa=1$) for linear invariant features \citep{hein2017formal}; the $t^{2n+1}$ example above is the large-$\kappa$ regime, and for the power-spectrum surrogate both arms are explicit (verified numerically, realized $\kappa\approx1.6$--$2.4$; on a radial cone the upper arm is certifiable a priori, with $\kappa\le2Bh_+/(h_-r_0\sigma_0)$, Proposition~\ref{prop:ps-alpha}). The certificate chain of \citet{tsuzuku2018lipschitz} (their Eq.~5) bounds $r_2$ only from below, through successively tighter Lipschitz constants; the upper bound $r_2\le\eta/\alpha$ is the matching arm that turns the one-sided certificate into a two-sided bracket. A tight converse of a different kind is already known for randomized smoothing, where \citet{cohen2019certified} show the smoothing radius is the largest the class probabilities alone can certify and is exact for linear bases; our bracket is instead deterministic and tied to a single fixed classifier, with looseness controlled by the explicit condition number $\kappa$ rather than by a worst case over classifiers. The full proof, with the explicit reaching-path and co-Lipschitz hypotheses, is in Appendix~\ref{app:bracket-lazy}.
\end{proposition}
We therefore read $\eta/L$ as a certificate and empirical predictor of robustness (\S\ref{sec:exp}) and, up to $\kappa$, as a characterization. One consequence deserves its own statement. The certificate is a statement about a \emph{budget}: a point survives every perturbation of norm at most $\varepsilon$ whenever its first-order radius clears $\varepsilon$. Within one threat model at one budget, models can therefore be compared by $\eta/L$ alone, and that is how the dissections of \S\ref{sec:exp} use it. Across \emph{different} budgets the raw ratio is the wrong comparison, because a harder budget lowers robust accuracy while adversarial training at that budget raises the raw margin; the natural cross-budget form of the threat-matched criterion is the \emph{budget-normalized} ratio $\eta/(L\varepsilon)$, the certified radius measured in units of the attack. The data-axis sweep of \S\ref{sec:exp} confirms this empirically: across adversarial-training budgets $\varepsilon\in\{2,4,8,16\}/255$ the raw threat-matched ratio anti-correlates with robust accuracy ($-0.95$) while the budget-normalized ratio collapses the whole sweep onto one monotone curve (Pearson $+0.94$). The distributional form is exact in the locally affine regime: on affine cells the certificate is an exact survival test, so robust accuracy equals the certified fraction of points whose per-sample radius clears $\varepsilon$ (Proposition~\ref{prop:count}, Appendix~\ref{app:count}), and the budget-normalized mean law is its scale-family shadow (Proposition~\ref{prop:scalefam}).
...
The structural theory says robust shift-invariant classifiers exist; it does not say gradient descent finds them. This section is a mostly negative result with one clean positive corollary: the two textbook testbeds for ``does invariance re-point the implicit bias'' are degenerate (in opposite ways), and in the lazy regime the invariant and unconstrained minimum-norm certificates \emph{coincide} on orbit-closed data, so any empirical robustness edge from invariance is a realized finite-width effect and not a smaller certificate; the trajectory question itself stays open. A reader tracking the $\eta/L$ diagnostic can take that sentence and move to \S\ref{sec:exp}. For unconstrained two-layer ReLU networks on near-orthogonal cluster data, the implicit bias of gradient descent provably selects a non-robust max-margin solution even though robust ones exist \citep{vardi2022gradient,frei2023double}, via \emph{feature averaging} \citep{li2025feature}; \citet{min2024can} conjecture that changing the architecture re-points this bias, and a natural hope is that hard-wired shift-invariance is such a lever. Testing that hope needs orbit-structured data on which the non-robustness premise genuinely holds. We characterize exactly when it can, via the rank of an orbit, and find that the two canonical orbit constructions both fail to, in opposite ways.
...
\paragraph{Statistical power.} Because each condition above has eight cells, we repeat the CIFAR-10 dissection on denser grids and report bootstrap $95\%$ confidence intervals (resampling cells) with a two-sided permutation test. On a $24$-cell standard-training grid (the four arms across six widths, four seeds averaged per cell), shift-consistency is not a significant predictor of the robust radius (Pearson $-0.33$, CI $[-0.68,+0.09]$, $p=0.11$), consistent with the near-zero MNIST and Fashion-MNIST values, whereas $\eta/L$ predicts it almost exactly ($+0.998$, CI $[+0.997,+0.999]$, $p<10^{-3}$). The $\eta/L$--radius value is high by construction: $\eta/L$ is a first-order lower bound on $r_2$ and these models are close to locally linear, so this correlation is the across-cell face of the per-sample certificate (\S\ref{sec:exp}) rather than a separate finding. The load-bearing result is the dissociation, that the architectural axis summarized by shift-consistency carries no information about the radius while the margin-to-sensitivity axis carries almost all of it. On a $12$-cell adversarially trained grid (the four arms across three widths, three seeds averaged per cell) the anti-prediction is significant: shift-consistency versus AutoAttack is $-0.75$ (CI $[-0.92,-0.02]$, $p=0.006$) and the threat-matched ratio is $+0.83$ (CI $[+0.49,+0.93]$, $p=0.002$), with the mismatched ratio not significant ($+0.27$, $p=0.39$) and no gradient masking. The strong consistency anti-prediction is thus specific to adversarial training.

\paragraph{Scaling to a ResNet backbone.} To check that the dissection is not an artifact of small networks, we repeat it on a PreActResNet-18 backbone on CIFAR-10 and CIFAR-100. We keep the four capacity-matched arms and add two parameter-matched low-invariance arms (a zero-padding variant and an explicitly aliased max-pool variant) at two widths and up to three seeds; here the exactly invariant arm uses adaptive polyphase sampling (APS) \citep{chaman2021truly} rather than circular padding, so the exact-invariance result is reproduced through a second mechanism. All arms are PGD-adversarially trained at $\ell_\infty{=}8/255$ with best-robust early stopping \citep{rice2020overfitting} and evaluated with AutoAttack (Figure~\ref{fig:resnet-at}). Both laws carry over at scale. Over the twelve architecture-by-width cells of these six operators the threat-matched ratio $\eta/\|\nabla M\|_1$ predicts AutoAttack robust accuracy with Pearson $+0.91$ ($p<10^{-3}$; $+0.77$ over the twenty-four individual trained models with a cell-resampling bootstrap), and under standard training $\eta/L$ predicts the $\ell_2$ robust radius with Pearson $+0.96$; shift-consistency anti-predicts robust accuracy with Pearson $-0.81$ ($p=0.002$), and this persists when we control for clean accuracy (partial correlation $-0.82$). Figure~\ref{fig:resnet-at} plots these alongside a seventh, held-out operator (TIPS, introduced below), fourteen cells in all, and the correlations are essentially unchanged by its inclusion (matched $+0.91$, consistency $-0.79$). The threat-matched ratio itself carries robustness information \emph{beyond} clean accuracy, and does so stably across arm subsets: its partial correlation with AutoAttack robust accuracy controlling for clean accuracy is $+0.89$ over the twelve cells, $+0.88$ with the exactly invariant arm removed, $+0.85$ over the four capacity-matched arms, and $+0.90$ over the three natural (standard, anti-aliased, shift-augmented) arms alone. So, unlike the shift-consistency anti-prediction, which strengthens only as the consistency range widens, the $\eta/L$ prediction does not depend on the degenerate exactly invariant operator, and $\eta/L$ is not a restatement of clean accuracy even though, as a single-operator selection rule, the two are comparable (\S\ref{sec:exp}). Across the four capacity-matched arms alone the anti-prediction is weak and not significant ($-0.43$); it sharpens to $-0.81$ only once the two low-invariance arms widen the consistency range. This is the point rather than a caveat: shift-consistency relates to robustness only across a broad enough span of the invariance axis and, even then, more weakly and with the opposite sign to what the anti-aliasing reading would suggest, whereas the threat-matched $\eta/L$ predicts robustness throughout. No gradient masking appears (AutoAttack stays below matched PGD for every cell, including the non-differentiable adaptive-polyphase arm). On CIFAR-100 the threat-matched law holds in the same direction (Spearman $+1.0$ over the four arms), underpowered at four cells.

Why shift-consistency is the unreliable signal is visible in the consistency axis itself: a ResNet backbone is already strongly shift-invariant, so over the four capacity-matched arms consistency is squeezed into $[0.93,1.00]$ and stops discriminating, and only once the two low-invariance arms widen it to $[0.85,1.00]$ does the anti-prediction reappear. The margin-to-Lipschitz ratio does not saturate this way and separates the arms throughout, so on a strong backbone shift-consistency is nearly constant while $\eta/L$ still tracks robustness.

\paragraph{An ImageNet-resolution check without the clean-accuracy confound.} To test the dissection beyond CIFAR statistics we repeat it on ImageNet-100 ($126{,}689$ training images, $100$ classes, $160$ pixels) with an ImageNet-stem ResNet-18 and the same four capacity-matched arms, adversarially trained with single-step Fast adversarial training \citep{wong2020fast} at $\ell_\infty{=}4/255$ (the ImageNet standard) for $15$ epochs, two seeds, and evaluated with AutoAttack at $4/255$ (PGD-$40$ masking check passed in every cell; the exactly invariant arm again uses adaptive polyphase sampling, exact to all integer circular shifts). Both laws hold at this scale, directionally at four arms per seed but consistently across seeds: pooled over the eight arm-by-seed models, the threat-matched $\eta/\|\nabla M\|_1$ predicts AutoAttack robust accuracy at Pearson $+0.90$ (Spearman $+0.86$) and shift-consistency anti-predicts it at $-0.87$. The new information is what this condition removes: clean accuracy is essentially constant across arms ($0.641$ to $0.649$, correlation with robust accuracy $+0.08$), yet AutoAttack spreads from $0.295$ to $0.342$, with the exactly invariant arm lowest through the lowest threat-matched ratio in both seeds. At CIFAR scale the exactly invariant arm's robustness deficit was entangled with a clean-accuracy deficit; here it fits as well as every other arm and is still the least robust, so the deficit is the margin geometry, not underfitting.

\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figures/resnet_at_predict_cifar10.pdf}
\caption{PGD-adversarially trained PreActResNet-18 on CIFAR-10 (fourteen architecture-by-width cells including the ported TIPS operator, bold; seeds, faint). Left: the $\ell_\infty$-matched ratio $\eta/\|\nabla M\|_1$ tracks AutoAttack robust accuracy at $\ell_\infty{=}8/255$ (Pearson $+0.91$); the TIPS cells (orange) fall on the same line. Right: shift-consistency does not order robustness (Pearson $-0.79$): the exactly invariant adaptive-polyphase arm (red) has consistency $1.0$ and the lowest robust accuracy, the high-consistency TIPS arm (orange) is mid-pack, well above that exactly invariant arm, and the most robust arms are the low-invariance ones (gray, purple) at low consistency.}
\label{fig:resnet-at}
\end{figure}

\paragraph{A graded anti-aliasing control at matched clean accuracy.} The arms compared so far also differ in how well they fit, so we add a controlled sweep that varies anti-aliasing strength while holding clean accuracy fixed. We PGD-adversarially train the capacity-matched CIFAR dissection backbone of Table~\ref{tab:cifar-at} with blur-pool filters of increasing width (Rect-2, Tri-3, Bin-5, Bin-7), which raise shift-invariance at nearly constant clean accuracy, together with the standard (aliased) and exact-cyclic endpoints (six arms, two widths, five seeds; Figure~\ref{fig:graded}). Three things follow. First, across the anti-aliasing family clean accuracy is matched within each width-and-seed cell to about one point (range $0.010$), yet robust accuracy rises ($0.390\to0.404$): the anti-aliasing arms exceed the aliased baseline in $39$ of $40$ cells (paired gain $+0.013$, bootstrap $95\%$ CI $[+0.011,+0.014]$; the strongest, Bin-5, exceeds standard by $+0.015$, paired $t$-test $p<0.001$), and the gain tracks a paired margin gain ($\eta:+0.062$, positive in every cell) at a roughly constant Lipschitz term. At matched fitting, anti-aliasing buys robustness through the margin, the opposite term to the one it acts on under standard training (where it instead lowers the Lipschitz constant, Figure~\ref{fig:cifar-decomp}). So invariance can help, and here it does so with no clean-accuracy cost. Second, shift-consistency is non-monotonic in robustness: it is nearly flat ($\approx0.80$) across the anti-aliasing family and then jumps to $1.0$ at the exact-cyclic arm, which is the \emph{least} robust of the six, so higher consistency does not imply higher robust accuracy, whereas the threat-matched $\eta/L$ orders the arms by robustness (Pearson $+0.92$). Third, and we state this plainly, the exact-cyclic arm's robustness deficit cannot be separated from a clean-accuracy deficit: forcing exact invariance collapses the margin ($\eta:1.53\to0.85$), and that one collapse lowers clean accuracy ($0.73\to0.58$) and robust accuracy together, so across the six arms clean accuracy predicts robustness as strongly as $\eta/L$ does. The right reading is therefore not that clean accuracy is a confounder to be removed, but that the margin is the upstream quantity both depend on: $\eta/L$ tracks robustness because it is built on the margin the invariance operator changes, while shift-consistency, which ignores the margin, does not.

\begin{figure}[t]\centering
\includegraphics[width=0.92\linewidth]{figures/graded_antialias.pdf}
\caption{Graded anti-aliasing under PGD adversarial training (six capacity-matched arms; cell means bold, seeds faint). The anti-aliasing family (Rect-2 to Bin-7, blue) is matched on clean accuracy to within one point. Left: the threat-matched $\eta/L$ orders all six arms by AutoAttack robust accuracy (Pearson $+0.92$). Right: shift-consistency does not: it is nearly flat across the anti-aliasing family and is $1.0$ for the exact-cyclic arm (red), which is the least robust, because exact invariance collapses the margin. Clean accuracy co-varies with robustness across the arms through that same margin collapse.}
\label{fig:graded}
\end{figure}

\paragraph{A held-out operator from the contrary literature: TIPS.} The closest work claiming the opposite correlation, that better shift-invariance goes with better adversarial robustness, is the learnable Translation Invariant Polyphase Sampling operator (TIPS) \citep{saha2024improving}. We port TIPS as a further arm of the ResNet dissection, faithfully as a downsampling operator at the same three sites as the other arms (its auxiliary shift-consistency losses are omitted so that the shared PGD adversarial-training recipe isolates the operator; TIPS adds two depthwise convolutions, so this arm is near- rather than exactly capacity-matched, $+0.16\%$ parameters). The ported operator accordingly inherits neither the full method's consistency gain nor its weak-attack advantage under standard training (Table~\ref{tab:weakattack}), consistent with the auxiliary losses rather than the operator driving those reported gains; our claims concern the operator. Two results. First, under adversarial training the TIPS operator is a \emph{good} operator: robust accuracy $0.417$ at width $1.0$ (three seeds, $\pm0.004$) and $0.390$ at width $0.5$ (two seeds), comparable to anti-aliased blur-pooling, with high consistency ($0.95$) and no gradient masking (confirmed on the full $10$k test set). Its robustness is exactly what the ratio predicts: the $\eta/L$ law fitted on the twelve cells \emph{without} TIPS predicts its AutoAttack accuracy to $+0.012$ and $-0.011$ at the two widths, an out-of-sample test on an operator we did not design. With TIPS included (fourteen cells, seven operators) the threat-matched ratio predicts robust accuracy at Pearson $+0.91$ (partial correlation $+0.89$ controlling for clean accuracy) while consistency remains anti-correlated ($-0.79$). Second, TIPS sharpens the consistency claim: consistency is uninformative in \emph{both} directions. The exactly invariant adaptive-polyphase arm has consistency $1.0$ and the lowest robust accuracy; TIPS has nearly the same consistency ($0.95$) yet sits mid-pack, well above it, while the most robust arms are the least consistent. Knowing a model's shift-consistency tells one nearly nothing about its robustness, while its threat-matched $\eta/L$ places every operator on the same line.

\paragraph{The contrary correlation lives only under weak attacks without adversarial training.} The invariance-helps-robustness reports \citep{saha2024improving,wang2025bridging} measure standard-trained models under gradient-based attacks, the fast gradient sign method (FGSM) and PGD (rotation- and scale-equivariant operators rather than pooling operators in the latter case), without AutoAttack and without adversarial training. We reproduce that evaluation regime on the shift axis, on our capacity-matched arms plus TIPS (standard training, FGSM/PGD/AutoAttack at $\varepsilon\in\{1,2,4,8\}/255$; Table~\ref{tab:weakattack}). Under FGSM at $1$--$2/255$ the invariance-helps ordering \emph{does} appear: the exactly invariant and shift-augmented arms sit above the standard baseline. Under AutoAttack the same models are at most $0.008$ robust accuracy at $\varepsilon=2/255$, for every arm including TIPS, across two seeds; \citet{saha2024improving} evaluate at budgets up to $192/255$, where AutoAttack robust accuracy of any standard-trained arm is a fortiori zero, so our grid bounds theirs from below. Two follow-ups close the remaining gaps. First, training the TIPS operator \emph{with} its auxiliary losses restores its weak-attack advantage (FGSM at $1/255$: $0.493$ versus the standard arm's $0.477$), and AutoAttack still reduces it to $0.004$ at $2/255$: the auxiliary losses produce the reported FGSM gain, and the gain is a weak-attack artifact either way. Second, we train the rotation-equivariant architecture of \citet{wang2025bridging} itself, with their published code and their exact recipe: their weak-attack numbers reproduce (FGSM $28.5\%$ at their headline budget), their own-protocol PGD-$40$ already collapses to $1.75\%$, and threat-matched AutoAttack at that budget leaves $0.1\%$. The ordering those works report does appear, but it is a property of weak attacks on undefended models, a regime with no adversarial robustness for any operator or symmetry group tested; under adversarial training, where robustness is nonzero, the ordering is governed by the threat-matched $\eta/L$ and not by invariance.

\paragraph{The same law holds for a data intervention, not only an architectural one.} Every dissection so far varies the \emph{architecture}. As a further, independent stress test we instead vary the training \emph{data}: we PGD-adversarially train the backbone ($\ell_\infty$, $8/255$) on CIFAR-10 augmented with $0$, $100$k, $500$k, or $1$M diffusion-generated images \citep{wang2023better}, compute-matched (identical optimizer steps per epoch and a fixed thirty-percent real fraction per batch), over three seeds with best-robust early stopping \citep{rice2020overfitting}. That synthetic data improves adversarially trained robustness is known \citep{wang2023better,gowal2021improving}; what we add is the $\eta/L$ accounting. At each model's best robust checkpoint the threat-matched ratio $\eta/\|\nabla M\|_1$ tracks AutoAttack robust accuracy across the twelve dose-by-seed models (Pearson $+0.85$, monotone in dose), while the mismatched $\ell_2$ ratio anti-predicts it ($-0.71$) and instead tracks the $\ell_2$ certified radius ($+0.98$): the same threat-matching seen along the architecture axis (Figure~\ref{fig:diffusion}). The four doses at this fixed budget span a narrow $\eta/L$ band; the budget sweep below traces the law across a wide one. The data axis also exposes, more sharply than the architecture axis, two ways a margin-versus-Lipschitz reading misleads. First, gauge: as the organizing principle of \S\ref{sec:intro} sets out, the split of $\eta/L$ into a margin term and a Lipschitz term is gauge-dependent (Lemma~\ref{lem:ratiodegen}), and the data axis makes this concrete: the same dose's ``smaller margin, smaller Lipschitz'' attribution reverses sign between a logit-norm anchor and a temperature anchor. Second, robust overfitting: at a fixed final epoch the comparison is confounded, because the real-only baseline robustly overfits, its input-gradient norm inflating late while its robust accuracy decays, which manufactures an apparent smoothness gain and makes even the mismatched ratio track robustness. Best-robust early stopping removes the confound: the robustness gain persists (AutoAttack $0.38\to0.48$) but now at equal or higher $L$, and the only quantity that still predicts robust accuracy is the threat-matched $\eta/L$. Read through the ratio, robust overfitting is itself an $\eta/L$ collapse that the added data prevents.
...
\paragraph{The mechanism is dynamical, and coverage-gated.} The gain has a mechanism we can name and test. It is dynamical: synthetic data delays robust overfitting. Across the helping doses the robust-accuracy peak moves later (from epoch $20$ for real-only to $40$ for the augmented arms, Spearman $+0.94$ in synthetic amount; Figure~\ref{fig:mechanism}, left) and the late $\eta/L$ collapse the real-only baseline suffers (its $\eta/L_2$ falls from $1.08$ at its robustness peak near epoch $20$ to $0.86$ at the final epoch) is absent in the augmented arms, which reach their robustness peak at the final epoch and hold their $\eta/L$, so the model reaches a higher best-robust checkpoint before it memorizes the finite adversarial training set \citep{rice2020overfitting}. The gain also requires sufficient diversity. At a fixed real fraction, pools of $1$k or $10$k unique images give no gain (best-checkpoint AutoAttack $0.35$ to $0.38$, at or below the real-only $0.38$) while $100$k and above do ($0.45$ to $0.48$); this is a coverage threshold rather than a mixing artifact, because a $10$k pool still yields no gain when its synthetic batch share is reduced from seventy to thirty percent ($0.38$, essentially baseline) whereas $100$k helps at both shares ($0.44$ to $0.45$; Figure~\ref{fig:mechanism}, right). A too-small pool can additionally \emph{hurt} when over-mixed, the $10$k arm falling below baseline only at the seventy-percent share. The operative requirement for a gain is therefore sufficient vicinal coverage, for which Appendix~\ref{app:vicinal} gives a candidate static account, while the realized effect is the prevention of the dynamical $\eta/L$ collapse.

\paragraph{Tracing the law across attack budgets.} To trace the data-axis law across a wide $\eta/L$ range rather than four nearby doses, we sweep the adversarial-training budget: $\varepsilon\in\{2,4,8,16\}/255$, real-only versus $1$M synthetic, two seeds each (sixteen models), every model evaluated \emph{threat-matched at its own training budget} (AutoAttack and the PGD masking check at that $\varepsilon$; AutoAttack $\le$ PGD in all sixteen cells) at its best-robust checkpoint, selected on a held-out split. The sweep spans $\eta/L_1$ from $0.024$ to $0.065$ and AutoAttack accuracy from $0.23$ to $0.82$, and it separates the two readings of the criterion cleanly (Figure~\ref{fig:wideband}, left). The \emph{raw} threat-matched ratio anti-correlates with robust accuracy across budgets ($-0.95$): training at a harder budget raises the raw margin while the harder evaluation lowers accuracy. The \emph{budget-normalized} ratio $\eta/(L_1\varepsilon)$ of \S\ref{sec:dich} collapses all sixteen models onto one monotone curve (Pearson $+0.94$, Spearman $+0.94$). The data intervention's offset is also quantified here: at matched $\eta/L_1$ the $1$M-synthetic models sit $+0.10$ AutoAttack above the real-only models (regression dummy $+0.10$, $t\approx+5.0$, robust to controlling for $\varepsilon$), and the offset behaves exactly as the robust-overfitting mechanism predicts (Figure~\ref{fig:wideband}, right): the synthetic arms have a uniformly smaller train--test robust gap, and both the gap and the gain shrink together as $\varepsilon$ grows, from a $+0.10$ to $+0.13$ gain where the real-only baseline overfits badly ($\varepsilon\le8/255$) to $+0.02$ at $\varepsilon=16/255$ where there is little overfitting left to prevent. The best-robust epoch tells the same story at every budget: the real-only arms peak at epochs $12$ to $34$, the synthetic arms at $34$ to $38$. Read per sample rather than through means, the certificate accounts for the offset as well: the \emph{certified fraction}, the share of held-out test points whose per-sample threat-matched radius $M(x)/\|\nabla M(x)\|_1$ clears the training budget, predicts AutoAttack accuracy at $+0.998$ across all sixteen models, and conditioning on it absorbs the data intervention's offset entirely (the dose coefficient falls from $+0.053$ under the mean ratio to $+0.006$, $t=0.78$). Consistently with this, the per-sample radius distributions are approximately a common shape rescaled across models, and the synthetic data's effect at matched location is a thinner lower tail, fewer barely-non-robust points. The mean ratio is thus a sufficient summary within one intervention but not across interventions; the per-sample certificate count is sufficient for both here. Appendix~\ref{app:count} formalizes all three readings: on affine cells robust accuracy \emph{equals} the certified fraction (Proposition~\ref{prop:count}), a scale family makes any scale proxy collapse the sweep (Proposition~\ref{prop:scalefam}), and a mean-preserving contraction of the radius distribution raises exactly the tail functional robust accuracy measures (Lemma~\ref{lem:mpc}, Proposition~\ref{prop:vic-tail}).

\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figures/wideband_law.pdf}
\caption{The adversarial-training-budget sweep ($\varepsilon\in\{2,4,8,16\}/255$, real-only circles versus $1$M-synthetic squares, two seeds each; every model evaluated threat-matched at its own training budget). Left: the budget-normalized ratio $\eta/(L_1\varepsilon)$ collapses all sixteen models onto one monotone curve (Pearson $+0.94$); the raw ratio anti-correlates across budgets ($-0.95$) because harder training raises the margin while harder evaluation lowers accuracy. Within each budget the synthetic squares sit above the real-only circles at similar $\eta/(L_1\varepsilon)$, the data intervention's generalization offset. Right: that offset tracks the robust overfitting it removes: the train--test robust gap is uniformly smaller with synthetic data, and the AutoAttack gain of the synthetic arms (annotations) shrinks with the gap as $\varepsilon$ grows.}
\label{fig:wideband}
\end{figure}
...
\paragraph{The two regimes.} Whether shift-invariance helps or harms adversarial robustness has been argued in both directions. On the harming side, \citet{ge2021shift} prove that a fully shift-invariant classifier can be far less robust than a fully connected one. On the helping side, reducing aliasing has been shown to improve adversarial robustness in practice \citep{grabinski2022aliasing,grabinski2023fix,rodriguezmunoz2022aliasing}, exact alias-free architectures attain shift-invariance with certified robustness to translation attacks \citep{michaeli2023alias}, and anti-aliased or learned-invariant pooling improves shift-consistency together with clean accuracy and corruption robustness \citep{zhang2019making,saha2024improving}, and most recently \citet{wang2025bridging} build rotation- and scale-equivariant defenses and argue, through an orbit-invariant Lipschitz bound, that symmetry improves certified robustness. These findings have not been placed on a common axis; we do so through whether the discriminative signal is preserved under projection onto the invariant subspace with positive margin, summarized by $\eta/L$. We also test the closest contrary claims directly (\S\ref{sec:exp}): the invariance-helps ordering of \citet{saha2024improving} reproduces under FGSM on standard-trained models but collapses to zero under AutoAttack already at budgets below those used in that literature, the regime where those correlations are measured; and under adversarial training the TIPS operator is genuinely robust, with its robustness predicted out of sample by the threat-matched $\eta/L$ and not by its (high) shift-consistency. A related trade-off line shows that geometric (spatial) invariance and adversarial robustness cannot both be maximized \citep{kamath2021canwehaveitall,tramer2019multiple}, and that more invariance raises vulnerability to invariance-based attacks \citep{tramer2020fundamental}; in our terms these are the small-$\eta/L$ and large-$\rho_G$ corners of the same criterion (\S\ref{sec:dich}). A model's invariance to a stochastic pre-processing defense's randomization can likewise erode the robustness that defense provides \citep{gao2022limitations}.

\paragraph{What we add.} The Lipschitz--margin certificate $r_2\ge\eta/L$ is classical \citep{hein2017formal,tsuzuku2018lipschitz}, with deterministic \citep{leino2021globally} and randomized-smoothing \citep{cohen2019certified} descendants, and the per-sample link between logit margin and input-space margin has been documented in robust models \citep{ngnawe2024detecting}. Our contribution is not the bound but the shift-invariance dissection it supports: that shift-consistency does not predict robustness while a threat-matched $\eta/L$ does, and that adversarial training inverts the within-model margin--sensitivity coupling. The orbit-invariant Lipschitz mechanism of \citet{wang2025bridging} is, in this taxonomy, the Lipschitz half of the criterion: it can lower the sensitivity term but is silent on the margin, and our dissection shows that lowering sensitivity alone does not order robustness (their certified-robustness argument is theoretical; their empirical evidence is FGSM/PGD, and retraining their rotation-equivariant architecture with their own code and recipe, we find its weak-attack robustness does not survive converged attacks, \S\ref{sec:exp}). Our lazy-regime theorem (Appendix~\ref{app:bracket-lazy}) shares their observation that orbit-averaging smooths (suppresses variation in) the gradient field, but shows that on orbit-closed data the invariant and unconstrained minimum-norm certificates coincide (so any advantage is a realized, finite-width effect, not a norm gap), adds the matching upper bound on the radius, and gives the margin-side account of the regime where invariance instead destroys the margin and hurts. On the optimization side, gradient descent provably converges to non-robust networks \citep{vardi2022gradient,frei2023double,li2025feature}, \citet{min2024can} conjecture an architectural fix, and \citet{melamed2023adversarial} tie non-robustness to off-manifold gradients; the neural-tangent-kernel account of robust versus non-robust features \citep{tsilivis2022what} underlies our lazy-regime result, and invariance has separately been used to aid robustness directly, for instance through test-time equivariance \citep{mao2023robust}. Architecture-versus-robustness studies \citep{huang2023revisiting} vary kernel, normalization, depth, and width under strong attacks but not shift-consistency. The robustness background is \citet{madry2018towards,goodfellow2015explaining,tsipras2019robustness}.

\paragraph{Synthetic data for adversarial training.} A separate line improves adversarial training by adding generated data. Diffusion- and GAN-generated images raise robust accuracy and reduce robust overfitting on CIFAR-scale benchmarks \citep{gowal2021improving,rebuffi2021fixing,wang2023better}, which is the setting our data-axis experiment (\S\ref{sec:exp}) builds on. We do not propose a new generator or training recipe and do not compete on robust accuracy. What we add on this axis is the $\eta/L$ accounting: the extra data helps in a way the threat-matched ratio predicts, and the same intervention exposes two ways a margin-versus-Lipschitz reading of the gain misleads, through gauge choice and through the robust-overfitting confound.

\section{Limitations and open problems}
The central open problem is a trajectory-level implicit-bias theorem for shift-invariant convolution-plus-pooling networks on data where the non-robustness premise genuinely holds (Proposition~\ref{prop:rank} shows the canonical orbit constructions are degenerate, while Proposition~\ref{prop:generic} shows generic orbit data is not, so such data exists). The criterion $\eta/L$ is a lower-bound certificate with no general converse, so the true robust radius can exceed it; the power-spectrum Lipschitz constant grows with input norm, so the guarantee is normalization-dependent; for real filters the convolution-square-pool layer spans conjugate-pair power coordinates rather than individual frequencies; the orbit-gradient statement is exact only for the continuous group; and the empirical study covers standard and PGD-adversarial training across three datasets (with a CIFAR-100 check), two threat models, small-convolutional and PreActResNet-18 backbones, and an ImageNet-100 dissection under single-step Fast adversarial training (four arms per seed, so directional), so full ImageNet-1k, larger backbones, multi-step adversarial training at that scale, and the rich-regime trajectory analysis are left to future work. On the data axis, the mechanism we can verify is the prevention of a dynamical robust-overfitting $\eta/L$ collapse; the static vicinal-coverage account of Appendix~\ref{app:vicinal} is a falsifiable candidate for its coverage side rather than a confirmed generative mechanism, and the coverage threshold itself is characterized only empirically. Three further caveats bound the reading of the empirical results. First, the near-perfect $\eta/L$--radius agreement is partly definitional and should be read as a consistency check, not independent validation: $\eta/L$ is a first-order lower bound on the certified radius $r_2$ and the trained models are close to locally linear, so that correlation is expected; the load-bearing empirical content is instead the shift-consistency dissociation and the adversarial-training anti-correlation, which is why we control the $\eta/L$ prediction for clean accuracy (partial correlation $+0.89$) and report its stability across arm subsets. Second, the capacity-matched comparison is the CIFAR-10 and PreActResNet-18 dissection; the small-network MNIST and Fashion-MNIST sweeps vary capacity across arms and are supplementary. Third, $\eta/L$ predicts robustness \emph{within} an intervention axis but is not a single cross-axis constant: at matched $\eta/L_1$ the diffusion-data models sit measurably above the real-only models ($+0.10$ AutoAttack, $t\approx+5$ in the budget sweep), an offset that tracks the train--test robust gap the data removes and vanishes at $\varepsilon=16/255$ where there is no robust overfitting left to prevent. The mean test-set ratio is therefore not a sufficient statistic for robust accuracy across interventions; the per-sample certified fraction is (\S\ref{sec:exp}: it predicts AutoAttack at $+0.998$ and absorbs the offset), which localizes the limitation to the mean summary rather than to the certificate. The certified-fraction law and the tail mechanism are formalized in Appendix~\ref{app:count} under a locally-affine hypothesis; the $+0.998$ agreement indicates that regime holds at the budgets used, and its breakdown at larger budgets or curvier models is the open edge. The claim is that the same threat-matched ratio orders robustness on each axis, not that the two axes fall on one regression line.
...
Under $f\mapsto cf$ both $M$ and $\nabla M$ scale by $c$, leaving the ratio unchanged; the penalty $\mathbb E[\|\nabla M\|/M]\ge0$ attains its infimum $0$ only as $\nabla M\to0$, i.e.\ at a constant score, which classifies at chance. This is the constant-function degeneracy already noted by \citet{tsuzuku2018lipschitz} in the large-target limit $c\to\infty$; the scale-invariance route to it here is specific to the ratio $\eta/L$.
\end{proof}
...
\paragraph{A certifiable upper arm for quadratic invariants.} The proof above notes the upper arm is informative only when $\alpha$ is certified independently of $r_2$. The following results give a nonlinear class where it is: scores built on a quadratic invariant, with the reaching path, the feature co-Lipschitz constant, and the head slope all known a priori.

\begin{proposition}[Upper radius from a certifiable feature secant]\label{prop:secant-upper}
Let $g=h\circ\Psi$ with $g(x)>0$, and suppose there is a point $z$ with $g(z)=0$ such that $\|\Psi(x)-\Psi(z)\|_2\ge c_-\|x-z\|_2$ and $h(\Psi(x))-h(\Psi(z))\ge h_-\|\Psi(x)-\Psi(z)\|_2$ for constants $c_-,h_->0$ known independently of the robust radius. Then $r_2(x)\le g(x)/(h_-c_-)$; and if $L(x)$ is any certified local Lipschitz bound for $g$ on the relevant ball, $g(x)/L(x)\le r_2(x)\le g(x)/(h_-c_-)$, so the bracket condition number obeys $\kappa\le L(x)/(h_-c_-)$.
\end{proposition}
\begin{proof}
$z$ is feasible for the radius infimum, so $r_2(x)\le\|x-z\|_2$. The two secant bounds give $g(x)=g(x)-g(z)\ge h_-\|\Psi(x)-\Psi(z)\|_2\ge h_-c_-\|x-z\|_2$, hence $\|x-z\|_2\le g(x)/(h_-c_-)$. The lower arm is the certificate of Corollary~\ref{cor:C}.
\end{proof}

\begin{lemma}[Radial co-Lipschitz bound for the power spectrum]\label{lem:ps-colip}
Let $\Psi(x)=(|\hat x_k|^2)_k$ with the unitary DFT, and for $r_0,\sigma_0>0$ let $\mathcal C(r_0,\sigma_0,B)=\{x:r_0\le\|x\|_2\le B,\ \|\Psi(x/\|x\|_2)\|_2\ge\sigma_0\}$. For $x\in\mathcal C(r_0,\sigma_0,B)$ and $t\in[0,1]$,
\[
\|\Psi(x)-\Psi(tx)\|_2\ \ge\ r_0\sigma_0\,\|x-tx\|_2 .
\]
(The matching upper bound $\|\Psi(x)-\Psi(y)\|_2\le2B\|x-y\|_2$ on the ball is Lemma~\ref{lem:ps-lip}.)
\end{lemma}
\begin{proof}
Two-homogeneity gives $\Psi(tx)=t^2\Psi(x)$, so $\|\Psi(x)-\Psi(tx)\|_2=(1-t^2)\|\Psi(x)\|_2$ while $\|x-tx\|_2=(1-t)\|x\|_2$; the ratio is $(1+t)\,\|\Psi(x)\|_2/\|x\|_2\ge\|x\|_2\|\Psi(x/\|x\|_2)\|_2\ge r_0\sigma_0$ on the cone, using $\Psi(x)=\|x\|_2^2\Psi(x/\|x\|_2)$ and $1+t\ge1$.
\end{proof}

\begin{proposition}[Certifiable bracket for radial power-spectrum scores]\label{prop:ps-alpha}
Let $g=h\circ\Psi$ with $\Psi$ as above, $x\in\mathcal C(r_0,\sigma_0,B)$, $g(x)>0$, and $g(0)<0$. By continuity there is $t_\star\in(0,1)$ with $g(t_\star x)=0$; set $z=t_\star x$. If $h(\Psi(x))-h(\Psi(z))\ge h_-\|\Psi(x)-\Psi(z)\|_2$ for some $h_->0$, then $r_2(x)\le g(x)/(h_-r_0\sigma_0)$. If moreover $h$ is $h_+$-Lipschitz on $\Psi(B_2(0,B'))$ with $B'$ large enough that the certified neighborhood of $x$ stays in the ball, then
\[
\frac{g(x)}{2B'h_+}\ \le\ r_2(x)\ \le\ \frac{g(x)}{h_-r_0\sigma_0},
\qquad
\kappa\ \le\ \frac{2B'h_+}{h_-\,r_0\sigma_0}.
\]
\end{proposition}
\begin{proof}
$\varphi(t)=g(tx)$ is continuous with $\varphi(0)<0<\varphi(1)$, giving $t_\star$ and the boundary point $z=t_\star x$ on the explicit radial path. Lemma~\ref{lem:ps-colip} and the head secant give $g(x)=g(x)-g(z)\ge h_-r_0\sigma_0\|x-z\|_2$, so $r_2(x)\le\|x-z\|_2\le g(x)/(h_-r_0\sigma_0)$. For the lower arm, $|g(u)-g(v)|\le h_+\|\Psi(u)-\Psi(v)\|_2\le2B'h_+\|u-v\|_2$ on the ball by Lemma~\ref{lem:ps-lip}, and the standard certificate applies.
\end{proof}

\begin{remark}[Why this is not tautological]\label{rem:alpha-cert}
The upper arm here is not obtained by naming the unknown secant slope of the true minimal path $\alpha$: the reaching path is the explicit radial path $t\mapsto tx$, the feature co-Lipschitz constant is the explicit cone constant $r_0\sigma_0$, and $h_-$ is a property of the head on a known feature segment. All three are available before any attack is run, so the bracket is genuinely two-sided on the cone. This answers, for the quadratic-invariant class, the question of when $\eta/L$ determines the radius up to an a priori $\kappa$.
\end{remark}

\begin{theorem}[Lazy-regime self-symmetrization: no minimum-norm certified-Lipschitz gap on orbit-closed data]\label{thm:lazy-cluster}
...
\item therefore $L_{\mathrm{inv}}\le L_{\mathrm{unc}}$, and at the common unit margin $\eta$
  ...
  orbit-closed data there is \emph{no} strict minimum-norm gap. A strict advantage requires
  non-orbit-closed data; the cluster-geometry advantage seen empirically is a
...
norm gap exactly $\|(I-\Pi_G)f_{\mathrm{unc}}\|_K^2$. On orbit-closed data this gap is in fact
...
minimum-norm certificates coincide on orbit-closed data; the tied-versus-unconstrained
...
Table~\ref{tab:at-general} reports the correlations that summarize the generality experiment of \S\ref{sec:exp}; Table~\ref{tab:at-perarm} gives the per-arm AutoAttack robust accuracies behind them, with clean accuracy and circular-shift consistency alongside. These per-arm numbers make the MNIST saturation noted in \S\ref{sec:exp} explicit: at $\ell_\infty=0.3$ every arm is robust to within one point of $94\%$, so the across-arm spread is too narrow for the (still negative) consistency correlation to reach significance at this grid size. On Fashion-MNIST and $\ell_2$-CIFAR the spread is several times wider and the exactly invariant arm, whose consistency is $1.0$ by construction, is the least robust in each width.
...
\subsection{The weak-attack regime reproduces the invariance-helps reading}\label{app:weakattack}
Table~\ref{tab:weakattack} gives the numbers behind the weak-versus-strong contrast of \S\ref{sec:exp}: the capacity-matched ResNet arms plus TIPS, \emph{standard}-trained (no adversarial training), attacked with FGSM, PGD-10, and AutoAttack at small $\ell_\infty$ budgets, the protocol of the invariance-helps reports \citep{saha2024improving,wang2025bridging}. Under FGSM at $1/255$ the exactly invariant and shift-augmented arms sit above the standard baseline (the anti-aliased and ported-TIPS arms do not), reproducing that literature's ordering for the built-in-invariance arms; under AutoAttack every arm is at or near zero robust accuracy by $\varepsilon=2/255$, so the ordering ranks models that are all non-robust. FGSM overestimates robust accuracy here by more than a factor of $30$ relative to PGD-10, and by roughly a factor of $40$ relative to AutoAttack, at the same budget.

\begin{table}[h]\centering
\caption{Standard-trained arms under weak and strong attacks (width $1.0$; $\ell_\infty$ budgets in $/255$; AutoAttack on $512$ points, binomial $95\%$ half-width $\approx0.007$ near zero, so the collapse conclusion is insensitive to the sample; the ordering replicates at a second seed with AutoAttack $\le0.008$ for every arm). The FGSM ordering at $\varepsilon{=}1$ reproduces the invariance-helps reading for the built-in-invariance arms (exact-APS and shift-augmented above standard); adding TIPS's auxiliary losses restores its FGSM advantage as well; the AutoAttack columns show every arm is non-robust in this regime. The same holds beyond the shift group: the rotation-equivariant architecture of \citet{wang2025bridging}, retrained with their code and recipe, reproduces their weak-attack numbers (FGSM $28.5\%$ at their headline budget) yet retains $0.1\%$ under threat-matched AutoAttack.}
\label{tab:weakattack}
\footnotesize
...
\subsection{Certificate counts and the tail mechanism}\label{app:count}
These results formalize the per-sample readings of \S\ref{sec:exp}: the first-order radius $r_1(x)=M(x)/\|\nabla M(x)\|_1$ is, in the locally affine regime, an \emph{exact} survival test for the $\ell_\infty$ threat, so robust accuracy is a lower-tail functional of the $r_1$ distribution; the budget-normalized mean law is the scale-family shadow of that count; and the data-axis offset is a tail effect. Throughout, $M(x)=f_y(x)-\max_{j\neq y}f_j(x)$ and nonnegative margin means survival (a tie rule that fails at zero margin changes only the measure-zero tie boundary in the generic case).

\begin{proposition}[Exact first-order certificate count on affine cells]\label{prop:count}
Fix $\varepsilon>0$ and suppose $M$ is affine on $B_\infty(x,\varepsilon)$, so $M(x+\delta)=M(x)+\langle g,\delta\rangle$ there, with $g=\nabla M(x)$. Then
\[
\inf_{\|\delta\|_\infty\le\varepsilon}M(x+\delta)=M(x)-\varepsilon\|g\|_1,
\]
so $x$ survives every $\ell_\infty$ perturbation of radius $\varepsilon$ if and only if $M(x)\ge\varepsilon\|g\|_1$; equivalently (with $r_1=\infty$ when $g=0$ and $M>0$), if and only if $M(x)\ge0$ and $r_1(x)\ge\varepsilon$. If the model is affine on $B_\infty(x,\varepsilon)$ for almost every test point, the population robust accuracy equals the certified fraction,
\[
\mathrm{RobAcc}(\varepsilon)=\mathbb P_x\big(M(x)\ge0,\ r_1(x)\ge\varepsilon\big).
\]
\end{proposition}
\begin{proof}
By affineness, $\inf_\delta M(x+\delta)=M(x)+\inf_{\|\delta\|_\infty\le\varepsilon}\langle g,\delta\rangle$. H\"older gives $\langle g,\delta\rangle\ge-\varepsilon\|g\|_1$ for every feasible $\delta$, and the bound is attained by the feasible $\delta^\star_j=-\varepsilon\,\mathrm{sign}(g_j)$ (any value in $[-1,1]$ for $\mathrm{sign}(0)$), since $\langle g,\delta^\star\rangle=-\varepsilon\sum_j|g_j|$. Survival on the whole ball is nonnegativity of the infimum, i.e.\ $M(x)\ge\varepsilon\|g\|_1$; for $g\neq0$ this reads $r_1(x)\ge\varepsilon$, and for $g=0$ the margin is constant on the ball, so survival is $M(x)\ge0$, matching the convention. Taking probability over the test distribution gives the count.
\end{proof}

\begin{remark}[Reading]\label{rem:count-reading}
The forward implication is the margin-over-local-Lipschitz certificate specialized to the $\ell_\infty/\ell_1$ dual pair \citep{hein2017formal,tsuzuku2018lipschitz}; the content is the converse: on an affine cell the perturbation $-\varepsilon\,\mathrm{sign}(\nabla M)$ attains the dual norm exactly, so the certificate is an exact survival test, not merely a lower bound. For a multi-class score $M$ is a minimum of pairwise affine margins, so ``$M$ affine on the ball'' says the active rival does not switch there; the $+0.998$ agreement between the certified fraction and AutoAttack in \S\ref{sec:exp} indicates this regime holds at the budgets used.
\end{remark}

\begin{proposition}[Scale-family collapse]\label{prop:scalefam}
Let $R(x)=r_1(x)\mathbf 1_{\{M(x)\ge0\}}$ and $\mathrm{CF}(\varepsilon)=\mathbb P(R\ge\varepsilon)$. If, across a family of models indexed by $a$, the radii satisfy $R_a\overset d=\mu_aZ$ for a nonnegative $Z$ whose law does not depend on $a$ and scales $\mu_a>0$, then $\mathrm{CF}_a(\varepsilon)=\bar F_Z(\varepsilon/\mu_a)$ is a fixed monotone function of $\mu_a/\varepsilon$; any summary $T_a\propto\mu_a$ (the mean ratio, the median of $R_a$) then collapses the family onto one curve $\mathrm{CF}_a(\varepsilon)=\Phi(T_a/\varepsilon)$.
\end{proposition}
\begin{proof}
$\mathrm{CF}_a(\varepsilon)=\mathbb P(\mu_aZ\ge\varepsilon)=\bar F_Z(\varepsilon/\mu_a)$; set $\Phi(s)=\bar F_Z(1/s)$, monotone since $\bar F_Z$ is nonincreasing, and absorb the proportionality constant of $T_a$ into $\Phi$.
\end{proof}

\begin{lemma}[Mean-preserving contraction raises lower-tail survival]\label{lem:mpc}
Let $\mathbb EZ=0$, $\mu>0$, and $R_a=\mu+aZ$ for $a\ge0$. If $0\le a_2\le a_1$, then for every $\varepsilon\le\mu$, $\mathbb P(R_{a_2}\ge\varepsilon)\ge\mathbb P(R_{a_1}\ge\varepsilon)$; and whenever the $p$-quantile of $Z$ is nonpositive, $Q_{R_{a_2}}(p)\ge Q_{R_{a_1}}(p)$.
\end{lemma}
\begin{proof}
For $a=0$, $R_a=\mu$ a.s.\ and the survival probability is $1$, which is maximal. For $a>0$, $R_a\ge\varepsilon\iff Z\ge(\varepsilon-\mu)/a$; since $\varepsilon-\mu\le0$ and $a_2\le a_1$, $(\varepsilon-\mu)/a_2\le(\varepsilon-\mu)/a_1$, so the $a_2$-event contains the $a_1$-event. For quantiles, $Q_{\mu+aZ}(p)=\mu+aQ_Z(p)$, and $Q_Z(p)\le0$ with $a_2\le a_1$ gives the ordering.
\end{proof}

\begin{proposition}[Vicinal tail mechanism]\label{prop:vic-tail}
Suppose at a fixed training budget the test-point radius admits $R_q(x)=\mu(q)+a(q)Z(x)$ with $\mathbb EZ=0$, where $q$ is the vicinal coverage level, and that at matched location $\mu(q_1)=\mu(q_0)$ increased coverage suppresses the idiosyncratic component, $0\le a(q_1)\le a(q_0)$. Then $\mathbb P(R_{q_1}\ge\varepsilon)\ge\mathbb P(R_{q_0}\ge\varepsilon)$ for every $\varepsilon\le\mu$, and under the certificate identity of Proposition~\ref{prop:count} the same inequality holds for robust accuracy. In this model the coverage acts on robust accuracy only through the tail event $\{R_q\ge\varepsilon\}$, so no residual dose effect remains once the certified fraction is conditioned on.
\end{proposition}
\begin{proof}
Lemma~\ref{lem:mpc} with $a_1=a(q_0)$, $a_2=a(q_1)$ gives the tail inequality; Proposition~\ref{prop:count} converts it to robust accuracy; and since robust accuracy is a deterministic function of the tail event, conditioning on the certified fraction exhausts the path from $q$ to robust accuracy.
\end{proof}
This is the pattern of \S\ref{sec:exp}: the sweep collapses in the budget-normalized mean (the scale-family shadow), the synthetic data thins the lower tail at matched location, and the certified fraction absorbs the dose coefficient ($+0.053\to+0.006$).
