...
\paragraph{$\eta/L$ diagnoses but does not train: a controlled intervention.} We add to adversarial training a penalty on the threat-matched margin gradient $\|\nabla M\|_1$, the quantity the criterion is built on, and compare it against three controls: vanilla adversarial training, the standard $\ell_2$ loss-gradient penalty of \citet{ross2018improving,finlay2021scaleable}, and the same margin penalty with the mismatched $\ell_2$ norm (standard backbone, two widths, two seeds; Figure~\ref{fig:tmreg}). At the diagnostic level the intervention does exactly what the criterion predicts: only the threat-matched $\ell_1$ penalty raises $\eta/L$ (the input-gradient $\ell_1$ norm falls from $38$ to $22$ as the penalty strengthens), while the mismatched $\ell_2$ and loss-gradient penalties leave it unchanged, a causal confirmation that the matched norm is the one that moves the matched ratio. The higher $\eta/L$ does not, however, translate into robustness: AutoAttack robust accuracy is statistically unchanged for every moderate penalty (paired $p>0.4$ against adversarial training) and falls under the strongest matched penalty ($-0.011$, $p=0.026$), with no gradient masking. The reason is the self-limiting coupling of \S\ref{sec:exp}: penalizing the gradient lowers the margin in lockstep ($\eta$ falls from $1.58$ to $1.03$), so the ratio rises by shrinking numerator and denominator together while the absolute margin that robustness needs erodes. Maximizing the ratio directly is worse still: penalizing the margin-normalized sensitivity $\|\nabla M\|_1/M$ drives the classifier to the trivial constant solution, with margin and sensitivity both vanishing and accuracy at chance, the maximal-invariance and zero-margin degeneracy the theory anticipates \citep{tsuzuku2018lipschitz}. The ratio $\eta/L$ is thus a faithful cross-model diagnostic but a poor training objective, a distinction the intervention makes precise.
