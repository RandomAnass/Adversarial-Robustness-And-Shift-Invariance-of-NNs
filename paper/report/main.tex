...
\begin{abstract}
Shift-invariance is a defining inductive bias of convolutional networks, yet its effect on adversarial robustness is contested. \citet{ge2021shift} prove that a fully shift-invariant classifier can be far less robust than a fully connected one, while a line of architectural work finds that reducing aliasing, which makes a network more shift-invariant, improves robustness \citep{grabinski2022aliasing,saha2024improving}. We argue these are two regimes of one question: whether the discriminative signal is preserved when the data is projected onto the invariant feature space, and with what margin relative to the feature's sensitivity. For a shift-invariant linear classifier the answer is exact: the classifier reads the data only through its projection onto the constant (DC) component, with a margin identity that recovers the $1/\sqrt{\dd}$ collapse of \citet{ge2021shift}. A convolution-plus-pooling layer with a nonlinearity instead computes power-spectrum features, which separate frequency-coded classes that no linear classifier can, and a degree-three bispectrum invariant separates even localized, phase-coded classes at the cost of a larger Lipschitz constant; so invariance can also help. We summarize the robustness that remains by a margin-to-Lipschitz ratio $\eta/L$ and the Lipschitz--margin certificate $r_2\ge\eta/L$, which we complete to a two-sided bracket $\eta/L\le r_2\le\eta/\alpha$ under an explicit co-Lipschitz constant (condition number $\kappa=L/\alpha$, exact for linear features). The same scalar reconciles the apparent counterexample of \citet{kamath2021canwehaveitall}, whose discriminative signal is sign-coded and therefore discarded by invariance, leaving a vanishing invariant margin. Empirically, across standard and adversarial training, three datasets, two threat models, and a PreActResNet-18 backbone, a threat-matched $\eta/L$ predicts the adversarial robust radius and AutoAttack robust accuracy (Pearson up to $+0.9$), whereas shift-consistency does not, and under adversarial training it anti-predicts robustness (as low as $-0.9$). The certificate holds per sample, with $M/\|\nabla M\|_2$ predicting each input's own robust radius, and adversarial training inverts the within-model coupling between margin and input sensitivity. On the optimization side we give the exact rank of a shift orbit, show that the two canonical orbit constructions are degenerate testbeds, and prove that shift-invariance is margin-free on orbit-closed data, where gradient descent self-symmetrizes; on near-orthogonal cluster data the invariant architecture reaches a larger $\eta/L$, which we prove in the lazy/NTK regime.
\end{abstract}
