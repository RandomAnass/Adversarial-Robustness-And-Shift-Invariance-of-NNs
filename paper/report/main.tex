...
\begin{abstract}
Making a convolutional network more shift-invariant has been reported both to reduce its adversarial robustness \citep{ge2021shift} and to improve it \citep{grabinski2022aliasing,saha2024improving}. We show these are two regimes of one quantity, a margin-to-Lipschitz ratio $\eta/L$: the discriminative margin that survives projection onto the invariant feature space, divided by that feature's sensitivity measured in the attack's dual norm (which we call \emph{threat-matched}). A large ratio lets invariance and robustness coexist; a small surviving margin forces a trade-off. The ratio is gauge-free (unchanged when the logits are rescaled), so robustness attaches to $\eta/L$ and not to the margin or the Lipschitz constant alone.

The practical consequence is a dissociation. Shift-consistency, the invariance metric the anti-aliasing literature tracks, does not order adversarial robustness, and under adversarial training it anti-predicts robustness: the exactly invariant operator has perfect consistency yet the lowest robust accuracy. The threat-matched $\eta/L$ predicts robustness instead, across three datasets, two threat models, a PreActResNet-18 backbone, and ImageNet-100 resolution with clean accuracy matched across architectures, and it predicts out of sample the robustness of a learnable-invariance operator from the contrary literature. That contrary ``invariance helps'' result is an artifact of weak attacks: retraining those models under their own recipe, it reproduces under the fast gradient sign method and vanishes under a strong attack.

We give the structural theory behind $\eta/L$ (the invariant linear margin is the separation of the data projected onto the invariant subspace, recovering the $1/\sqrt\dd$ collapse of \citet{ge2021shift}; a power-spectrum layer that escapes it; a two-sided robustness bracket), and show that $\eta/L$ is a diagnostic to read off a trained model, not a target to train toward: because the ratio is scale-invariant its direct maximizer is the constant classifier, and four first-order attempts to raise it all collapse the margin and leave robustness flat or worse.
\end{abstract}
...
\item \textbf{A unifying criterion.} The threat-matched $\eta/L$ places the ``invariance harms'' and ``invariance helps'' results on one axis (\S\ref{sec:dich}). It is a robustness certificate $r_2\ge\eta/L$ that we complete to a two-sided bracket under an explicit reachability condition, and it is gauge-free, so robustness attaches to the ratio and not to the margin or Lipschitz term alone.
\item \textbf{The dissociation (main result, \S\ref{sec:exp}).} In a capacity-matched architectural dissection, shift-consistency does not order adversarial robustness and anti-predicts it under adversarial training, while the threat-matched $\eta/L$ predicts it. This holds across three datasets, two threat models, a PreActResNet-18 backbone, and ImageNet-100 resolution with clean accuracy matched across architectures, and $\eta/L$ carries robustness information beyond clean accuracy (partial correlation $+0.89$). Fitted without a held-out learnable-invariance operator, it predicts that operator's robustness out of sample.
\item \textbf{The contrary literature is a weak-attack artifact (\S\ref{sec:exp}).} Retraining the two operators reported to make invariance help robustness \citep{saha2024improving,wang2025bridging} under their own code and recipe, their advantage reproduces under the fast gradient sign method and vanishes under AutoAttack.
\item \textbf{$\eta/L$ is a diagnostic, not a trainable target (\S\ref{sec:coupling}).} The invariant linear margin is the separation of the data projected onto the invariant subspace (Theorem~\ref{thm:A}), recovering the $1/\sqrt\dd$ collapse of \citet{ge2021shift}; an exact polar identity ties the margin gradient to the margin; and because the ratio is scale-invariant its maximizer is the constant classifier. Four first-order attempts to train $\eta/L$ all collapse the margin. Extended results (an optimization-side no-gap theorem, and a training-\emph{data} axis via diffusion-augmented adversarial training) are in the appendix.
\end{itemize}
\paragraph{One organizing principle.} The ratio $\eta/L$ is \emph{gauge-free}: rescaling the score $f\mapsto cf$ multiplies both the margin $\eta$ and the local Lipschitz constant $L$ by $c$, leaving the ratio (and the input-space radius it lower-bounds) unchanged (Lemma~\ref{lem:ratiodegen}). The margin and the Lipschitz constant are individually \emph{gauge-dependent}, so attributing a robustness change to ``larger margin'' or ``smaller Lipschitz'' separately is well-posed only once a gauge is fixed. We therefore anchor the robustness attribution on the gauge-invariant quantities, $\eta/L$ and the certified radius, and read the empirical margin/Lipschitz split (\S\ref{sec:exp}) at the training-fixed logit scale, where the per-term decomposition is well-defined. This single principle organizes the paper: invariance changes robustness only through $\eta/L$, and in the orbit-closed regime where it cannot move the minimum-norm $\eta/L$ at all, any empirical advantage is a property of the \emph{realized} finite-width solution rather than of the norm certificate. It also reaches past architecture: intervening on the training \emph{data} instead of the architecture, through diffusion-augmented adversarial training, obeys the same threat-matched law within that axis, a second and independent test of the diagnostic (Appendix~\ref{app:dataaxis}). And it is why $\eta/L$ is a diagnostic rather than a trainable target (\S\ref{sec:coupling}).
...
\section{Additional experimental results: the data axis, per-sample structure, and selection}\label{app:dataaxis}
...
\section{Small-network dissection (MNIST, Fashion-MNIST)}\label{app:smallnet}
These capacity-varying small-convolutional sweeps are supplementary to the capacity-matched CIFAR-10 and PreActResNet-18 dissections of the main text.
