...
The certificate $r_2\ge\eta/L$ makes $\eta/L$ a target one might hope to train. We argue it is a diagnostic rather than a trainable objective, from three angles: a homogeneity identity puts a structural floor under the input-gradient norm (so the coupling is not purely an optimizer artifact), the ratio is scale-invariant (so its direct maximizer is degenerate), and four controlled interventions confirm that every attempt to lower or maximize it collapses the margin (Appendix~\ref{app:tmreg}). For a $k$-class score $f$ with true label $y$, write $M(x)=f_y(x)-\max_{j\neq y}f_j(x)$ for the signed logit margin and $\eta=\mathbb E[M]$, $L=\mathbb E\|\nabla M\|$ for its dataset means, the same $(\eta,L)$ the dissection of \S\ref{sec:exp} measures.

\begin{remark}[Homogeneity envelope and capacity ceiling]\label{rem:coupling}
For a bias-free network whose activations are positively homogeneous of degree one (ReLU, leaky-ReLU, absolute value, maxout), $M$ is degree-1 homogeneous, so Euler's identity $\langle\nabla M(x),x\rangle=M(x)$ holds almost everywhere; with Cauchy--Schwarz it gives the lower envelope $\|\nabla M(x)\|_2\ge M(x)/\|x\|_2$ (threat-matched $\|\nabla M(x)\|_1\ge M(x)/\|x\|_\infty$), so $\eta/L\le\sup_i\|x_i\|_2$. A larger margin forces a larger gradient floor, so the margin--sensitivity coupling is not purely an optimizer artifact. Two caveats keep this in proportion. The envelope is \emph{loose at the operating point}: for the trained models $\|\nabla M\|_2$ sits well above $M/\|x\|_2$, so it does not by itself forbid raising $\eta/L$. And the identity is exact only for bias-free homogeneous networks, so on the PreActResNet-18 models, which carry biases and batch normalization, it is an idealized mechanism rather than an exact law. An architecture-independent reading of the same ceiling follows the law of robustness \citep{bubeck2021law}: applied to the clipped margin $\mathrm{clip}(M/\eta,-1,1)$, a $p$-parameter interpolator of $n$ points in $\R^\dd$ has $\eta/L\le\tilde O(\sqrt{p/(n\dd)})$, which depends only on $(p,n,\dd)$ and not on the activation, consistent with the coupling holding for ReLU, SiLU, and GELU alike (order of magnitude $\sqrt{p/(n\dd)}\approx0.27$ for a PreActResNet-18 on CIFAR-10; the law bounds the global Lipschitz constant under label noise, so we read it as capacity context rather than a tight account of the local ratio).
\end{remark}
...
Together, Remark~\ref{rem:coupling} and Lemma~\ref{lem:ratiodegen} predict a no-go for first-order training of $\eta/L$: penalizing $\|\nabla M\|$ must shrink the margin, and maximizing the ratio collapses to the constant classifier. Appendix~\ref{app:tmreg} confirms this with four controlled interventions, all of which collapse the margin exactly as predicted.
...
\paragraph{$\eta/L$ is a diagnostic, not a training target.} A natural next step is to train $\eta/L$ directly. It does not work. On a standard backbone, four controlled interventions that penalize or maximize the ratio, namely an isotropic threat-matched gradient penalty, direct ratio maximization, an anisotropic hinge penalty, and a directional penalty at the adversarial point, each move $\eta/L$ at the diagnostic level but collapse the margin in lockstep, leaving AutoAttack robust accuracy flat or worse with no gradient masking, exactly as Remark~\ref{rem:coupling} and Lemma~\ref{lem:ratiodegen} predict. Appendix~\ref{app:tmreg} reports the interventions in full.
