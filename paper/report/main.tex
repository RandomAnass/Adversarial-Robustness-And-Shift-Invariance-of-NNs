...
\paragraph{A practical reading: $\eta/L$ as an attack-free selection signal.} The dissection turns $\eta/L$ into a usable diagnostic. Computed from a trained model on clean inputs alone, as the mean active margin divided by the mean input-gradient norm, $\eta/L$ ranks the invariance operators by their adversarial robust accuracy (Pearson $+0.93$ on the graded sweep, $+0.91$ on the ResNet backbone) without running any attack, whereas shift-consistency, the standard invariance diagnostic, ranks the least robust operator first. A practitioner choosing an anti-aliasing or invariance operator for robustness should therefore compare $\eta/L$, not shift-consistency or clean shift-stability. This is a selection signal rather than a defense; whether directly regularizing $\eta/L$ during training is itself effective is the question we turn to next.
