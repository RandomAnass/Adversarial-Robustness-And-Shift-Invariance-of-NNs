...
holds for any finite separable dataset and does not require orbit-closure.
\end{theorem}
\begin{proof}
...
$\lVert\gamma(T)-x\rVert_2\le\eta/\alpha$. Since $\gamma(T)\in\mathcal B$ has
$\sign g(\gamma(T))\neq y$, and by continuity points immediately beyond
$\gamma(T)$ have strictly opposite sign, the perturbation $\delta=\gamma(T)-x$ is
label-flipping and $r_2\le\lVert\gamma(T)-x\rVert_2\le\eta/\alpha$.
...
then $r_2(x)\le g(x)/(h_-r_0\sigma_0)$. If moreover $h$ is $h_+$-Lipschitz on
...
\begin{corollary}[The direct maximizer is the constant classifier]\label{cor:ratiodegen-app}
...
absolute margin and admits the constant classifier $\grad M\equiv0$ as a global
optimizer, so $\etaL$ is ill-posed as a training objective and well-posed only as
a diagnostic read at a fixed logit scale.
\end{corollary}
\begin{proof}
The penalty $\E[\lVert\grad M\rVert/M]\ge0$ attains its infimum $0$ only as
$\grad M\to0$, i.e.\ at a constant score, which classifies at chance. Because the
ratio is scale-invariant (Lemma~\ref{lem:ratiodegen}) the objective cannot
distinguish a large-margin robust classifier from a degenerate flat one; this is
the constant-function degeneracy noted by \citet{tsuzuku2018lipschitz} in the
large-target limit, reached here through scale-invariance.
\end{proof}
...
$\ell_2$ radius (DDN, mismatched) & --- & $+0.48$ & $+0.09$ \\
\midrule
...
$\ell_2$ radius (DDN, mismatched) & --- & $-0.74$ & $-0.26$ \\
\bottomrule
\end{tabular}
\end{table}
