...
RobustBench models at Spearman $-0.79$, and $-0.77$ after controlling for clean accuracy. We identify the
mechanism, an effective-dimension margin deficit, and separate it from the curvature account. Finally, the
...
per image; it is a certificate $r\ge\etaL$ that we complete to a two-sided bracket
$\etaL\le r\le\eta/\alpha$ with an explicit reachability slope $\alpha$.
...
$A=\lVert\grad M\rVert_1/\lVert\grad M\rVert_2$ does, at Spearman $-0.77$ over thirty RobustBench models
after a clean-accuracy control. We identify the mechanism, an effective-dimension margin deficit
$r\approx(\eta/L_1)/(1+CA)$, show it is transverse rather than on-direction curvature, and place it as a
relaxation of \citet{simongabriel2019adversarial}.
...
just a bound but tight, essentially the radius itself. For linear features the two sides coincide ($\kappa{=}1$,
$r=\eta/\lVert w\rVert$, recovering \citet{hein2017formal}); we verify numerically that the true radius
lies strictly inside the bracket with $\kappa\in[1.6,2.4]$. This completes the missing upper arm of the
margin-Lipschitz bound of \citet{tsuzuku2018lipschitz}. The gauge-freeness and the structural theory
...
\begin{itemize}[leftmargin=1.2em,itemsep=2pt]
\item \textbf{CIFAR-10, standard training.} Across a dense $24$-cell capacity-matched grid, $\etaL$
orders the $\ell_2$ robust radius at Pearson $+0.998$ (bootstrap CI $[+0.997,+0.999]$, $p<10^{-3}$);
shift-consistency does not (Pearson $-0.33$, CI $[-0.68,+0.09]$, $p=0.11$).
...
prediction is not a restatement of clean accuracy.
...
itself (partial $-0.65$). Restricted to the twenty-nine robust models, dropping the single non-robust
one, the ranking is $-0.77$ (Figure~\ref{fig:aniso}a). The size $\eta/L_1$ detects robustness but ranks it only weakly
...
input gradient \citep{participation2025}, used there to \emph{detect} catastrophic overfitting during
training rather than to rank models across a family. The synthetic model in Eq.~\ref{eq:deficit} is a
...
a trainable target: because the ratio is scale-invariant its direct maximizer is the constant classifier
...
robustness; the anti-aliasing line \citep{zhang2019making,grabinski2022aliasing,saha2024improving} and
the equivariance line \citep{wang2025bridging} report the opposite sign. These locate as positions on the
threat-matched $\etaL$ axis: Ge's collapse and the trade-off results are the small-margin corner, where
invariance costs margin and $\etaL$ falls, while the ``helps'' reports are a weak-attack reading that does
not survive AutoAttack rather than a third regime.
...
full AutoAttack. The deficit law of Eq.~\ref{eq:deficit} is a proposed, well-controlled mechanism rather
...
This appendix states the three load-bearing results; full proofs, the power-spectrum escape, and the
lazy-regime cluster theorem are in the accompanying technical report.

The first result identifies the margin $\eta$ that enters $\etaL$ once invariance is imposed. It says
that a shift-invariant linear classifier separates classes only through their average (DC) component,
so the surviving margin is the separation of the DC-projected data, which recovers and generalizes the
$1/\sqrt\dd$ collapse of \citet{ge2021shift}.

\begin{theorem}[Invariant linear margin equals projection separation]\label{thm:A}
A linear score $g(x)=w^\top x+b$ is shift-invariant for all $x$ if and only if $w=c\mathbf 1$.
Consequently, for any finite separable classes $X_+,X_-$, the invariant $\ell_2$ max-margin equals the
separation of the DC-projected data,
$\gamma_{\mathrm{inv}}=\tfrac12\,\mathrm{dist}(\mathrm{conv}\,\Pinv X_+,\ \mathrm{conv}\,\Pinv X_-)$,
with maximizing direction $\pm\mathbf 1/\sqrt\dd$. This holds for any finite separable dataset and does
not require orbit-closure.
\end{theorem}

The single-image case, where each class is one image and all its shifts, is the observation of
\citet{ge2021shift}; the theorem adds the exact convex-hull form for an arbitrary finite dataset, which
is what the architectural dissection of \S\ref{sec:etaL} measures. The same accounting holds for any
finite symmetry group, with the DC projector replaced by the group-average projector.

The second result upgrades the certificate $r\ge\etaL$ to a two-sided bracket, so $\etaL$ is a
characterization of the radius up to a condition number rather than only a lower bound.

\begin{proposition}[Two-sided bracket: a conditional converse]\label{prop:sandwich}
If the score is $L$-Lipschitz toward the boundary and admits a co-Lipschitz (lower-Lipschitz) constant
$\alpha$ along a path reaching it, then $\eta/L\le r_2\le\eta/\alpha$ with condition number
$\kappa:=L/\alpha\ge1$, so $\etaL$ determines the robust radius up to $\kappa$. The bracket is exact
($\kappa=1$, $r_2=\eta/\lVert w\rVert_2$) for linear invariant features \citep{hein2017formal}; for the
power-spectrum surrogate both arms are explicit and the realized $\kappa\in[1.6,2.4]$ numerically. This
completes the certificate chain of \citet{tsuzuku2018lipschitz}, which bounds $r_2$ only from below,
with the matching upper arm.
\end{proposition}

The third result is the organizing lemma behind gauge-freeness, and behind why $\etaL$ is a diagnostic
rather than a training target (\S\ref{sec:signflip}).

\begin{lemma}[Scale-invariance of the ratio]\label{lem:ratiodegen}
The per-sample ratio $M(x)/\lVert\grad M(x)\rVert$ is invariant to positive rescaling of the score,
$f\mapsto cf$, since numerator and denominator scale together. Hence an objective that maximizes $\etaL$
directly is blind to the absolute margin and admits the constant classifier $\grad M\equiv 0$ as a global
optimizer, so $\etaL$ is ill-posed as a training objective and well-posed only as a diagnostic read at a
fixed logit scale.
\end{lemma}

\section{Additional experiments}\label{app:extra}
The technical report gives the following, summarized here.
\emph{Cross-budget normalization.} Across adversarial-training budgets
$\varepsilon\in\{2,4,8,16\}/255$ the raw threat-matched ratio anti-correlates with robust accuracy
($-0.95$) while the budget-normalized ratio $\eta/(L\varepsilon)$, the certified radius in units of the
attack, collapses the sweep onto one monotone curve (Pearson $+0.94$); within a fixed budget, which is
how the dissections compare models, the raw ratio is the right form.
\emph{A data-axis test.} Intervening on the training data instead of the architecture, through
diffusion-augmented adversarial training, obeys the same threat-matched law, an independent axis for the
diagnostic.
\emph{$\etaL$ is not a trainable target.} Four first-order attempts to raise $\etaL$ (Lemma
\ref{lem:ratiodegen}) all collapse the margin and leave robustness flat or worse.
\emph{The anisotropy range study.} On MNIST and Fashion-MNIST a fixed training budget compresses the
anisotropy $A$ into a narrow band ($[1.9,2.9]$ and $[4.3,5.4]$) where it does not rank robustness. An
adversarial-training-strength sweep widens the band (to $[2.4,16.6]$ on MNIST, $[4.0,13.2]$ on Fashion)
and restores the ranking law of \S\ref{sec:aniso}: over twenty-four cells per dataset, $A$ orders
threat-matched $\ell_\infty$ robust accuracy at $-0.83$ ($\varepsilon{=}0.2$) and $-0.89$
($\varepsilon{=}0.3$) on MNIST and $-0.61$ ($\varepsilon{=}0.1$) to $-0.80$ ($\varepsilon{=}0.15$) on
Fashion, with every correlation recomputed three ways and no gradient masking. In the mismatched $\ell_2$
radius the sign is correct on Fashion ($-0.74$) but reverses on MNIST ($+0.48$ raw, null after controlling
clean accuracy), because strong high-$\varepsilon$ $\ell_\infty$ training over-fits the $\ell_\infty$ ball
and shrinks the $\ell_2$ radius; the full per-cell tables are here.
